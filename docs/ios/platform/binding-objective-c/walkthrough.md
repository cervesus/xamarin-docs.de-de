...
- Als vorkompiliertes Framework. Dabei handelt es sich nur um einen Ordner, der die statische Bibliothek, Header und manchmal zusätzliche Ressourcen mit `.framework` Erweiterung enthält. Beispielsweise [die AdMob-Bibliothek von Google](https://developers.google.com/admob/ios/download).
...
Wie bereits erwähnt, verwenden wir die Xcode-Befehlszeilen Tools auf dem **Mac-buildhost** (insbesondere `make` und `lipo`) in dieser exemplarischen Vorgehensweise. Der `make`-Befehl ist ein sehr häufiges UNIX-Hilfsprogramm, das die Kompilierung ausführbarer Programme und Bibliotheken mithilfe eines _Makefile_ -para gramms automatisiert, das angibt, wie das Programm erstellt werden soll. Der `lipo`-Befehl ist ein OS X-Befehlszeilen-Hilfsprogramm zum Erstellen von Dateien mit mehreren Architekturen. Es werden mehrere `.a` Dateien zu einer Datei zusammengefasst, die von allen Hardwarearchitekturen verwendet werden kann.
...
- **Installieren von Xcode** : Wenn Sie Xcode installieren, werden alle Ihre Befehlszeilen Tools gebündelt. In OS X 10,9 Shims (installiert in `/usr/bin`) kann jedes in `/usr/bin` enthaltene Tool dem entsprechenden Tool in Xcode zuordnen. Beispielsweise der `xcrun` Befehl, mit dem Sie ein beliebiges Tool in Xcode über die Befehlszeile suchen oder ausführen können.
- **Die Terminalanwendung** : Sie können die Befehlszeilen Tools über die Terminalanwendung installieren, indem Sie den Befehl "`xcode-select --install`" ausführen:
  - Starten Sie die Terminal Anwendung.
  - Geben Sie `xcode-select --install` ein, und drücken **Sie die Eingabe**Taste, z.b.:
...
- **[Erstellen einer statischen Bibliothek](#Creating_A_Static_Library)** : dieser Schritt umfasst das Erstellen einer statischen Bibliothek des Ziel-C-Codes von **infcolorpicker** . Die statische Bibliothek verfügt über die Dateierweiterung `.a` und wird in die .NET-Assembly des Bibliotheks Projekts eingebettet.
...
Laden wir das infcolorpicker-Projekt von [GitHub](https://github.com/InfinitApps/InfColorPicker/archive/master.zip) herunter, und entpacken Sie es in das Verzeichnis unserer Wahl. Wenn Sie das Xcode-Ziel für `PickerSamplePhone` Projekt öffnen, wird die folgende Projektstruktur im Xcode-Navigator angezeigt:
...
4. Geben Sie `InfColorPicker` für den **Projektnamen** ein, und klicken Sie auf die Schaltfläche **weiter** :
...
Öffnen Sie die Terminal Anwendung auf Ihrem Mac, und navigieren Sie zum Speicherort Ihrer Makefile. Geben Sie `make` in das Terminal ein, drücken **Sie die Eingabe** Taste, und das **Makefile** wird ausgeführt:

[![](walkthrough-images/lib01.png "Sample makefile output")](walkthrough-images/lib01.png#lightbox)

Wenn Sie make ausführen, wird ein großer Text durch Scrollen von angezeigt. Wenn alles ordnungsgemäß funktioniert, sehen Sie, dass die Wörter **Build erfolgreich** sind und die `libInfColorPicker-armv7.a`, `libInfColorPicker-i386.a` und `libInfColorPickerSDK.a` Dateien an denselben Speicherort wie das **Makefile**kopiert werden:
...
An diesem Punkt haben wir den ersten Schritt der IOS-Bindung abgeschlossen, indem wir eine statische Bibliothek mit Xcode erstellt haben und die Xcode-Befehlszeilen Tools `make` und `lipo`. Fahren Sie mit dem nächsten Schritt fort, und verwenden Sie "Target **-Sharpie** ", um die Erstellung der API-Bindungen für uns zu automatisieren.
...
1. Wählen Sie im Menü Datei die Option **neu** > Projekt **Mappe** **...** aus:

    ![](walkthrough-images/bind01.png "Starting a new solution")

1. Wählen Sie im Dialogfeld neue Projekt Mappe die Option **Bibliothek** > **IOS-Bindungs Projekt**aus:
...
1. Wählen Sie im Dialogfeld Neues Projekt die **Option C# Visual > iPhone & iPad > IOS-Bindungs Bibliothek (xamarin)** aus:
    ...
    [![wählen Sie die IOS-Bindungs Bibliothek aus.](walkthrough-images/bind02.w157-sml.png)](walkthrough-images/bind02.w157.png#lightbox)
...
1. Navigieren Sie zu der zuvor erstellten FAT-Binärdatei (`libInfColorPickerSDK.a`), und klicken Sie auf die Schaltfläche **Öffnen** :
...
1. Kopieren Sie die `libInfColorPickerSDK.a` von Ihrem **Mac-buildhost** , und fügen Sie Sie in das Bindungs Projekt ein.
...
Bevor wir den Bindungsprozess starten können, müssen wir den folgenden Befehl in die Terminal `sharpie xcode -sdks`eingeben, um die aktuellen installierten sdche zu erhalten:
...
Im obigen Beispiel sehen wir, dass das `iphoneos9.3` SDK auf dem Computer installiert ist. Nachdem diese Informationen vorhanden sind, können wir das infcolorpicker-Projekt `.h` Dateien in die anfängliche **ApiDefinition.cs** und `StructsAndEnums.cs` für das infcolorpicker-Projekt analysieren.
...
> [!TIP] 
> Übergeben Sie für das `-scope`-Argument den Ordner mit den Headern, die Sie binden möchten. Ohne das `-scope`-Argument versucht der Ziel-Sharpie, Bindungen für alle importierten IOS SDK-Header zu generieren, z. b. `#import <UIKit.h>`. Dies führt zu einer großen Definitionsdatei, die beim Kompilieren des Bindungs Projekts wahrscheinlich Fehler generiert. Wenn das `-scope`-Argument festgelegt ist, generiert Target Sharpie keine Bindungen für Header außerhalb des Bereichs bezogenen Ordners. 
...
Öffnen Sie beide Dateien im Bindungs Projekt, das wir oben erstellt haben. Kopieren Sie den Inhalt der Datei **InfColorPicker.cs** , und fügen Sie ihn in die Datei **ApiDefinition.cs** ein. ersetzen Sie dabei den vorhandenen `namespace ...` Codeblock durch den Inhalt der **InfColorPicker.cs** -Datei (belassen Sie die `using` Anweisungen unverändert):
...
Bei Ziel-Sharpie gibt es manchmal ein Problem beim Übersetzen `Delegates`. Daher müssen wir die Definition der `InfColorPickerControllerDelegate` Schnittstelle ändern und die `[Protocol, Model]` Zeile durch Folgendes ersetzen:
...
Wenn die Bindungs Bibliothek kompiliert wird, erstellt xamarin. IOS eine abstrakte Basisklasse mit dem Namen `InfColorPickerControllerDelegate`, die diese Schnittstelle mit virtuellen Methoden implementiert.
...
Beenden Sie die xamarin. IOS-Anwendung, indem Sie einen starken Delegaten verwenden, um auf die `colorPickerControllerDidFinish:` Nachricht zu reagieren:

**Subclass infcolorpickercontrollerdelegat** -fügt dem Projekt eine neue Klasse mit dem Namen `ColorSelectedDelegate`hinzu. Bearbeiten Sie die-Klasse, sodass Sie den folgenden Code enthält:
...
Xamarin. IOS bindet den Ziel-C-Delegaten, indem eine abstrakte Basisklasse mit dem Namen `InfColorPickerControllerDelegate`erstellt wird. Unterklasse dieses Typs, und überschreiben Sie die `ColorPickerControllerDidFinish`-Methode, um auf den Wert der `ResultColor`-Eigenschaft von `InfColorPickerController`zuzugreifen.

**Erstellen einer Instanz von colorselecteddelegat** : der Ereignishandler benötigt eine Instanz des `ColorSelectedDelegate` Typs, den wir im vorherigen Schritt erstellt haben. Bearbeiten Sie die-Klasse `InfColorPickerSampleViewController`, und fügen Sie der-Klasse die folgende Instanzvariable hinzu:
...
**Implementieren Sie die Methode "shandtouchupinsidewithstrongdelegat** -Next implementieren Sie den Ereignishandler für", wenn der Benutzer **colorchangebutton**berührt. Bearbeiten Sie `ViewController`, und fügen Sie die folgende Methode hinzu:
...
Zuerst wird eine Instanz von `InfColorPickerController` über eine statische Methode abgerufen, und diese Instanz muss über die Eigenschaft `InfColorPickerController.Delegate`den starken Delegaten erkennen. Diese Eigenschaft wurde von Ziel-Sharpie automatisch für uns generiert. Schließlich wird `PresentModallyOverViewController` aufgerufen, um die Ansicht `InfColorPickerSampleViewController.xib` anzuzeigen, damit der Benutzer eine Farbe auswählen kann.

**Ausführen der Anwendung** : an dieser Stelle wird der gesamte Code ausgeführt. Wenn Sie die Anwendung ausführen, sollten Sie in der Lage sein, die Hintergrundfarbe des `InfColorColorPickerSampleView` zu ändern, wie in den folgenden Screenshots zu sehen:

[![](walkthrough-images/run01.png "Running the Application")](walkthrough-images/run01.png#lightbox)

Glückwunsch! An diesem Punkt haben Sie eine Ziel-C-Bibliothek erfolgreich erstellt und für die Verwendung in einer xamarin. IOS-Anwendung gebunden. Als Nächstes erfahren Sie mehr über die Verwendung von schwachen Delegaten.
...
**ViewDidLoad aktualisieren** : Wir müssen `ViewDidLoad` ändern, damit der soeben erstellte Ereignishandler verwendet wird. Bearbeiten Sie `ViewController`, und ändern Sie `ViewDidLoad` so, dass er dem folgenden Code Ausschnitt ähnelt:
