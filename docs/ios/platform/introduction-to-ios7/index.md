...
IOS 7 erweitert die Animations Unterstützung in UIKit und ermöglicht Anwendungen das Ausführen von Aufgaben, die zuvor notwendig waren, direkt in das Core Animation Framework zu löschen. Beispielsweise `UIView` kann jetzt Spring-Animationen und Keyframe-Animationen ausführen, die zuvor `CAKeyframeAnimation` auf einen angewendet wurden `CALayer` .

### <a name="spring-animations"></a>Spring-Animationen

 `UIView`unterstützt jetzt das Animieren von Eigenschafts Änderungen mit Spring Effect. Um dies hinzuzufügen, geben Sie entweder die- `AnimateNotify` oder die `AnimateNotifyAsync` -Methode an, und übergeben Sie Werte für das Spring Dämpfung-Verhältnis und die anfängliche Spring Velocity, wie unten beschrieben:
...
Diese Spring Effect bewirkt, dass die Bildansicht beim Vervollständigen der Animation an einem neuen Center-Speicherort springt, wie unten dargestellt:
 ...
 ![Diese Spring Effect bewirkt, dass die Bildansicht springt, wenn Sie Ihre Animation an einem neuen Center-Speicherort abschließt.](images/spring-animation.png)

### <a name="keyframe-animations"></a>Keyframe-Animationen

Die- `UIView` Klasse enthält nun die- `AnimateWithKeyframes` Methode zum Erstellen von Keyframe-Animationen für einen `UIView` . Diese Methode ähnelt anderen `UIView` Animations Methoden, mit dem Unterschied, dass ein zusätzlicher `NSAction` als Parameter übergeben wird, um die Keyframes einzuschließen. Innerhalb `NSAction` von werden Keyframes durch Aufrufen von hinzugefügt `UIView.AddKeyframeWithRelativeStartTime` .
...
Sehen wir uns ein Beispiel an, in dem eine Schwerkraft und eine Kollisions Grenze zu einem hinzugefügt werden `UIView` .

#### <a name="uigravitybehavior"></a>Uigravitybehavior

Das Hinzufügen von Schweregrad zu einer Bildansicht folgt den drei oben beschriebenen Schritten.

Wir arbeiten in der- `ViewDidLoad` Methode für dieses Beispiel. Fügen Sie zunächst `UIImageView` wie folgt eine-Instanz hinzu:
...
Dadurch wird eine Bildansicht erstellt, die sich am oberen Rand des Bildschirms befindet. Erstellen Sie eine Instanz von, um das Bild mit der Schwerkraft zu machen `UIDynamicAnimator` :

```csharp
dynAnimator = new UIDynamicAnimator (this.View);
```

Das-Element verwendet eine `UIDynamicAnimator` Instanz eines Verweises `UIView` oder eine `UICollectionViewLayout` , die die Elemente enthält, die pro angefügtem Verhalten animiert werden.

Erstellen Sie als nächstes eine- `UIGravityBehavior` Instanz. Sie können ein oder mehrere Objekte übergeben, die implementieren, wie z. b. `IUIDynamicItem` `UIView` :

```csharp
var gravity = new UIGravityBehavior (dynItems);
```

Das Verhalten wird an ein Array von weitergeleitet `IUIDynamicItem` , das in diesem Fall die einzelne Instanz enthält, die `UIImageView` wir animieren.
...
![Der Speicherort des Start Images ](images/gravity2.png)
 ![](images/gravity3.png)

Da die Begrenzungen des Bildschirms nicht eingeschränkt sind, wird die Bildansicht einfach vom unteren Rand entfernt. Um die Ansicht so einzuschränken, dass das Bild mit den Rändern des Bildschirms kollidiert, können wir eine hinzufügen `UICollisionBehavior` . Dies wird im nächsten Abschnitt behandelt.

#### <a name="uicollisionbehavior"></a>Uicollisionbehavior

Wir beginnen mit der Erstellung eines `UICollisionBehavior` und fügen es dem dynamischen Animator hinzu, wie dies auch bei der der Fall war `UIGravityBehavior` .

Ändern Sie den Code so, dass er folgendes einschließt `UICollisionBehavior` :
...
Der `UICollisionBehavior` verfügt über eine-Eigenschaft mit dem Namen `TranslatesReferenceBoundsIntoBoundry` . Wenn diese Einstellung auf festgelegt `true` wird, werden die Begrenzungen der Verweis Ansicht als Kollisions Grenze verwendet.
...
Wir können das Verhalten der abstürzenden Bildansicht mit zusätzlichen Verhaltensweisen besser steuern. Beispielsweise könnten wir eine hinzufügen, `UIDynamicItemBehavior` um die Elastizität zu erhöhen, sodass die Bildansicht beim Konflikt mit dem unteren Rand des Bildschirms stärker springt.

Beim Hinzufügen eines werden `UIDynamicItemBehavior` die gleichen Schritte wie bei den anderen Verhaltensweisen befolgt. Erstellen Sie zuerst das Verhalten:
