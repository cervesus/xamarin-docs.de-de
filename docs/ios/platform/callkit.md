...
[![Der callkit-Dienst Stapel](callkit-images/callkit01.png)](callkit-images/callkit01.png#lightbox)
...
[![Monkeycallapp-Architektur](callkit-images/callkit02.png)](callkit-images/callkit02.png#lightbox)
...
[![Übermitteln von Berichten an das System mit einem cxcallcontroller](callkit-images/callkit04.png)](callkit-images/callkit04.png#lightbox)
...
Die `CallHandleFromURL` `CallHandleFromActivity` Klassen und werden im appdelegaten zum Abrufen des Kontakt Handles der Person verwendet, die in einem ausgehenden Aufruf aufgerufen wird. Weitere Informationen finden Sie weiter unten im Abschnitt [Verarbeiten von ausgehenden Aufrufen](#handling-outgoing-calls) .
...
Da dies nur eine Simulation ist, `ActiveCallManager` verwaltet nur eine Auflistung von `ActiveCall` -Objekten und verfügt über eine Routine, mit der ein angegebener-Befehl über seine-Eigenschaft gefunden werden kann `UUID` . Sie enthält außerdem Methoden zum Starten, beenden und Ändern des Zustands eines ausgehenden Aufrufes. Weitere Informationen finden Sie weiter unten im Abschnitt [Verarbeiten von ausgehenden Aufrufen](#handling-outgoing-calls) .

### <a name="the-providerdelegate-class"></a>Die providerdelegatklasse

Wie bereits erläutert, bietet eine bidirektionale `CXProvider` Kommunikation zwischen der APP und dem System für Out-of-Band-Benachrichtigungen. Der Entwickler muss ein benutzerdefiniertes bereitstellen `CXProviderDelegate` und an das Anfügen, `CXProvider` damit die app out-of-Band-callkit-Ereignisse verarbeiten kann. Monkeycallverwendet Folgendes `CXProviderDelegate` :
...
Wenn eine Instanz dieses Delegaten erstellt wird, wird die an Sie übermittelt, die für die Behandlung `ActiveCallManager` von Aufruf Aktivitäten verwendet wird. Anschließend werden die Handle-Typen () definiert, `CXHandleType` auf die der `CXProvider` antwortet:
...
Diese Werte werden in einem gebündelt `CXProviderConfiguration` , das zum Konfigurieren von verwendet wird `CXProvider` :
...
Der Delegat erstellt dann `CXProvider` mit diesen Konfigurationen einen neuen und fügt ihn an ihn an:
...
Wenn dies eine echte App wäre, wird die- `DidActivateAudioSession` Methode verwendet, um den-Befehl mit einem vorkonfigurierten zu starten, `AVAudioSession` den das System bereitgestellt hat:
...
Monkeycallverwendet den appdelegaten zum Speichern von Instanzen von `ActiveCallManager` und `CXProviderDelegate` , die in der gesamten App verwendet werden:
...
Die `OpenUrl` -Methode und die- `ContinueUserActivity` Überschreibungs Methode werden verwendet, wenn die APP einen ausgehenden-Befehl verarbeitet. Weitere Informationen finden Sie weiter unten im Abschnitt [Verarbeiten von ausgehenden Aufrufen](#handling-outgoing-calls) .
...
[![Ein Remote Benutzer hat eine VoIP-Konversation gestartet.](callkit-images/callkit05.png)](callkit-images/callkit05.png#lightbox)
...
Mit diesem Code wird eine neue `CXCallUpdate` -Instanz erstellt und ein Handle an das Objekt angehängt, das den Aufrufer identifiziert. Anschließend wird die-Methode der- `ReportNewIncomingCall` `CXProvider` Klasse verwendet, um das System über den-Befehl zu informieren. Wenn dies erfolgreich ist, wird der Aufruf der APP-Auflistung aktiver Aufrufe hinzugefügt, wenn dies nicht der Fall ist. der Fehler muss dem Benutzer gemeldet werden.
...
[![Der Benutzer antwortet auf den eingehenden VoIP-Rückruf.](callkit-images/callkit06.png)](callkit-images/callkit06.png#lightbox)

1. Die Benutzeroberfläche des Systems informiert das System darüber, dass der Benutzer den VoIP-Anruf beantworten möchte.
2. Das System sendet eine `CXAnswerCallAction` an die APP, die `CXProvider` Sie über die Antwort Absicht informiert.
...
Dieser Code sucht zuerst in der Liste der aktiven Aufrufe nach dem angegebenen Aufruf. Wenn der-Befehl nicht gefunden werden kann, wird das System benachrichtigt und die Methode beendet. Wenn Sie gefunden wird, `AnswerCall` wird die-Methode der- `ActiveCall` Klasse aufgerufen, um den Aufruf zu starten, und das System ist, wenn es erfolgreich ist oder fehlschlägt.
...
[![Der Benutzer beendet den-Befehl in der Benutzeroberfläche der app.](callkit-images/callkit07.png)](callkit-images/callkit07.png#lightbox)

1. Die App erstellt `CXEndCallAction` , die in einem gebündelt `CXTransaction` wird, das an das System gesendet wird, um Sie darüber zu informieren, dass der-Vorgang beendet wird.
2. Das System überprüft die Absicht des endanrufens und sendet die `CXEndCallAction` zurück an die APP über den `CXProvider` .
...
Dieser Code sucht zuerst in der Liste der aktiven Aufrufe nach dem angegebenen Aufruf. Wenn der-Befehl nicht gefunden werden kann, wird das System benachrichtigt und die Methode beendet. Wenn Sie gefunden wird, `EndCall` wird die-Methode der- `ActiveCall` Klasse aufgerufen, um den Aufruf zu beenden, und das System enthält Informationen, wenn es erfolgreich ist oder fehlschlägt. Bei erfolgreicher Ausführung wird der Aufruf aus der Sammlung aktiver Aufrufe entfernt.
...
In der obigen Situation sendet das System eine `CXTransaction` an die APP, die eine Liste mehrerer Aktionen (z `CXEndCallAction` `CXAnswerCallAction` . b. und) enthält. Alle diese Aktionen müssen einzeln ausgeführt werden, damit das System die Benutzeroberfläche entsprechend aktualisieren kann.
...
[![Empfangen einer beabsichtigte Start Absicht](callkit-images/callkit08.png)](callkit-images/callkit08.png#lightbox)
...
3. Wenn das System die Aktion annimmt, wird es über den Delegaten an die APP zurückgegeben `XCProvider` .
...
Hier wird die- `CallHandleFromActivity` Methode der-Hilfsklasse `StartCallRequest` verwendet, um das Handle für die aufgerufene Person zu erhalten (siehe die oben genannte [startcallrequest-Klasse](#the-startcallrequest-class) ).
...
Wenn eine- `CXEndCallAction` Klasse mit der UUID für den End-End-Befehl erstellt, bündeln Sie Sie in einem, der `CXTransaction` mithilfe der-Methode der-Klasse an das System gesendet wird `RequestTransaction` `CXCallController` .
...
[![Die Aktions Sequenz für den Start Vorgang.](callkit-images/callkit09.png)](callkit-images/callkit09.png#lightbox)

1. Eine Aktion zum Starten des Aufrufes wird von der APP empfangen, um einen eingehenden Anruf zu beantworten.
2. Bevor diese Aktion von der APP erfüllt wird, wird die Konfiguration bereitstellt, die für die erforderlich ist `AVAudioSession` .
3. Die APP informiert das System darüber, dass die Aktion erfüllt wurde.
4. Bevor der Aufruf eine Verbindung herstellt, bietet callkit eine hohe Priorität, `AVAudioSession` die der von der APP angeforderten Konfiguration entspricht. Die APP wird über die- `DidActivateAudioSession` Methode von benachrichtigt `CXProviderDelegate` .
...
3. Wählen Sie **IOS**  >  **Extensions**  >  **Aufrufe Verzeichnis Erweiterungen** aus, und klicken Sie auf die Schaltfläche **weiter** :
    ...
    [![Erstellen einer neuen Erweiterung für das Erweiterungs Verzeichnis](callkit-images/calldir01.png)](callkit-images/calldir01.png#lightbox)
4. Geben Sie einen **Namen** für die Erweiterung ein, und klicken Sie auf die Schaltfläche **weiter** :

    [![Eingeben eines Namens für die Erweiterung](callkit-images/calldir02.png)](callkit-images/calldir02.png#lightbox)
5. Passen Sie den **Projektnamen** und/oder Projektmappennamen bei Bedarf an, und klicken Sie auf die Schaltfläche **Erstellen** **Solution Name**
    ...
    [![Erstellen des Projekts](callkit-images/calldir03.png)](callkit-images/calldir03.png#lightbox)
...
3. Wählen Sie **IOS**  >  **Extensions**  >  **Aufrufe Verzeichnis Erweiterungen** aus, und klicken Sie auf die Schaltfläche **weiter** :
    ...
    [![Erstellen einer neuen Erweiterung für das Erweiterungs Verzeichnis](callkit-images/calldir01w.png)](callkit-images/calldir01.png#lightbox)
4. Geben Sie einen **Namen** für die Erweiterung ein, und klicken Sie auf **OK** .

-----

Dadurch wird `CallDirectoryHandler.cs` dem Projekt eine Klasse hinzugefügt, die wie folgt aussieht:
