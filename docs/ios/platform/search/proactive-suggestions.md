...
Wenn der Benutzer z. b. Text in ein Textfeld eingibt, das als `UITextContentType.FullStreetAddress`gekennzeichnet ist, kann das System die automatische Füllung des Felds mit dem Speicherort vorschlagen, den der Benutzer vor kurzem angezeigt hat.
...
Wenn eine Fahrt Freigabe-APP die [mkdirectionsrequest](xref:MapKit.MKDirectionsRequest) -API verwendet, wird Sie von IOS 10 als Option in der APP-Umstellung angezeigt, wenn der Benutzer wahrscheinlich eine Fahrt wünscht. Die APP muss auch als App für die Fahrt Freigabe registriert werden, indem die `MKDirectionsModeRideShare` für den [mkdirectionsapplicationsupportedmodes](https://developer.apple.com/library/content/documentation/General/Reference/InfoPlistKeyReference/Articles/iPhoneOSKeys.html) -Schlüssel in der `Info.plist`-Datei angegeben wird.
...
- **Handoff** - `NSUserActivity` wurde in ios 8 hinzugefügt, um die Übergabe zu unterstützen, die es dem Entwickler ermöglicht, eine Aktivität auf einem Gerät zu starten und dann auf einem anderen Gerät fortzufahren (Weitere Informationen finden Sie unter [Einführung in die Übergabe](~/ios/platform/handoff.md)).
- **Spotlight-Suche** : IOS 9 hat die Möglichkeit zum herauf Stufen von App-Inhalten aus den Spotlight-Suchergebnissen mithilfe von `NSUserActivity` hinzugefügt (siehe [Suchen mit Core Spotlight](~/ios/platform/search/corespotlight.md)).
- **Kontextabhängige Siri-Erinnerungen** : in ios 10 wurde `NSUserActivity` erweitert, damit Siri rasch eine Erinnerung zum Anzeigen der Inhalte erhalten kann, die der Benutzer zurzeit in der APP zu einem späteren Zeitpunkt anzeigen kann.
...
- **Interaktionen mit Interaktionen** : neu in ios 10 `NSUserActivity` ermöglicht, dass Kommunikations-apps von einer Kontaktkarte (in der App "Kontakte") als Alternative Kommunikationsmethode herauf gestuft werden.
...
Der Entwickler muss sicherstellen, dass es sich hierbei um denselben Aktivitätstyp Bezeichner (`com.xamarin.platform`) wie die oben erstellte Aktivität handelt. Die APP verwendet die im `NSUserActivity` gespeicherten Informationen, um den Zustand wiederherzustellen, an dem der Benutzer aufgehört hat.
...
5. Dies funktioniert sogar bei Drittanbieter-Apps (die `NSUserActivity`unterstützen), sodass der Benutzer zu einer Fahrt Freigabe-App wechseln kann, und die Adresse des Restaurants wird ebenfalls automatisch als Ziel vorgeschlagen.
...
Der Entwickler erstellt niemals direkt eine Instanz von `INIntent` oder `INIntentResponse`, er verwendet eine der untergeordneten Klassen (basierend auf der Aufgabe, die die APP im Namen des Benutzers ausführt), die von diesen übergeordneten Klassen geerbt werden. Beispielsweise `INSendMessageIntent` und `INSendMessageIntentResponse`, um eine Textnachricht zu senden. 

Wenn die Interaktion vollständig aufgefüllt ist, wenden Sie die `DonateInteraction`-Methode an, um das System zu informieren, dass die Interaktion zur Verwendung verfügbar ist.
...
Wenn Sie diesen Code ausführlich betrachten, erstellt er eine Instanz `NSUserActivity` und füllt ihn auf (wie im Abschnitt [Erstellen einer Aktivität](#creating-an-activity) oben gezeigt). Anschließend wird eine Instanz von `INSendMessageIntent` erstellt (die von `INIntent`erbt) und mit den Details der gesendeten Nachricht aufgefüllt:

```csharp
var intent = new INSendMessageIntent (to, text, "", "MonkeyChat", from);
```

Ein `INSendMessageIntentResponse` wird erstellt und an den oben erstellten `NSUserActivity` weitergeleitet:

```csharp
var response = new INSendMessageIntentResponse (INSendMessageIntentResponseCode.Success, activity);
```

Ein `INInteraction` wird aus den soeben erstellten Sende Nachrichten Absicht (`INSendMessageIntent`) und der Antwort (`INSendMessageIntentResponse`) erstellt:
...
Wie oben gezeigt, unterstützt `NSUserActivity` das System dabei, zu verstehen, welche Informationen der Benutzer zurzeit auf dem Bildschirm arbeitet. Schema.org fügt Webseiten ähnliche Möglichkeiten hinzu.
...
Die neue `TextContentType`-Eigenschaft der Textfeld-Steuerelemente in ios 10 ermöglicht es dem Entwickler, die semantische Absicht für den Wert zu definieren, der vom Benutzer in ein bestimmtes Feld eingegeben werden soll. Beispiel:
...
Im folgenden sind einige der gängigeren Typen aufgeführt, die für den Entwickler in der `UITextContentType` statischen Klasse verfügbar sind:
...
Wenn Sie einen einfachen API-Befehl in ios 10 einschließen, kann der Entwickler für die Benutzer der Medien-App eine ansprechkräftere Sperrbildschirm Darstellung erstellen. Wenn Sie die `MPPlayableContentManager`-Klasse zum Verwalten der Medienwiedergabe verwenden, werden vollständige mediensteuer Elemente (wie die von der Music-App dargestellten) auf dem Sperrbildschirm für die App angezeigt.
