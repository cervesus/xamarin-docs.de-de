...
Jede Methode verfügt über einen bestimmten Intent-Typ, der alle Informationen enthält, die Siri aus der Anforderung des Benutzers analysiert hat (und möglicherweise in der-Methode und der- `Resolve*` `Confirm*` Methode aktualisiert).
...
Die- `IntentHandler` Klasse ist der Einstiegspunkt für Siri-Anforderungen – jede Absicht wird an die-Methode übermittelt `GetHandler` , die ein Objekt zurückgibt, das die Anforderung verarbeiten kann.
...
> - Für das `Intents` Framework verfügen Typen über ein Präfix `IN*` (z. b. `INExtension`), aber dies sind _keine_ Schnittstellen.
> - Außerdem folgt die Verwendung von Protokollen (die zu Schnittstellen in c# werden) zwei `I` s, z `IINAddTasksIntentHandling` . b..
...
1. **Verarbeiten der Absicht** – die von Siri analysierten Daten werden in einem Objekt zur Verfügung gestellt `intent` , das für den Typ der Absicht spezifisch ist. Möglicherweise hat Ihre APP diese Daten mithilfe optionaler `Resolve*` Methoden überprüft.
...
Sie können einen benutzerdefinierten Aktivitätstyp festlegen, solange Ihre IOS-App ihn über den `NSUserActivityTypes` Schlüssel in " **Info. plist**" unterstützt. Sie können diesen Fall dann verarbeiten, wenn Ihre APP geöffnet wird, und bestimmte Vorgänge ausführen (z. b. das Öffnen eines relevanten Ansichts Controllers und das Laden der Daten aus dem Siri-Vorgang).

Im Beispiel wird auch das Ergebnis hart codiert `Success` , aber in realen Szenarien sollte die richtige Fehlerberichterstattung hinzugefügt werden.
