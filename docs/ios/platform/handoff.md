...
[![Ein Beispiel für das Ausführen eines Handoff-Vorgangs.](handoff-images/handoff02.png)](handoff-images/handoff02.png#lightbox)
...
`NSDocument`Für alle `UIDocument` Apps oder apps wird automatisch die Handoff-Unterstützung integriert, und es sind nur minimale Änderungen erforderlich, um die Übergabe zu unterstützen.

### <a name="continuing-user-activities"></a>Fortsetzen von Benutzeraktivitäten

Die `NSUserActivity` -Klasse (zusammen mit einigen kleinen Änderungen an `UIKit` und `AppKit` ) bietet Unterstützung für die Definition der Aktivität eines Benutzers, die auf einem der Geräte des Benutzers möglicherweise fortgesetzt werden kann.

Damit eine Aktivität an eine andere Geräte der Benutzer übertragen werden kann, muss Sie in einer Instanz gekapselt werden `NSUserActivity` , die als _aktuelle Aktivität_gekennzeichnet ist, deren Nutz Last festgelegt ist (die zum Ausführen der Fortsetzung verwendeten Daten), und die Aktivität muss dann an dieses Gerät übertragen werden.
...
Eine Instanz von `NSUserActivity` wird durch die `ActivityType` Eigenschaften und identifiziert `Title` . Die `UserInfo` Dictionary-Eigenschaft wird zum Übertragen von Informationen über den Status der Aktivität verwendet. Legen `NeedsSave` Sie die-Eigenschaft auf fest, `true` Wenn Sie die Zustandsinformationen mithilfe des-Delegaten verzögert laden möchten `NSUserActivity` . Verwenden Sie die- `AddUserInfoEntries` Methode, um nach Bedarf neue Daten von anderen Clients in das `UserInfo` Wörterbuch zusammenzuführen, um den Status der Aktivität beizubehalten.

### <a name="the-nsuseractivitydelegate-class"></a>Die nsuseractivitydelegatklasse

`NSUserActivityDelegate`Wird verwendet, um die Informationen in einem `NSUserActivity` - `UserInfo` Wörterbuch auf dem neuesten Stand zu halten und mit dem aktuellen Status der Aktivität synchron zu synchronisieren. Wenn das System die Informationen in der zu aktualisierenden Aktivität benötigt (z. b. vor der Fortsetzung auf einem anderen Gerät), wird die-Methode des-Delegaten aufgerufen `UserActivityWillSave` .

Sie müssen die `UserActivityWillSave` -Methode implementieren und Änderungen an der vornehmen `NSUserActivity` (z `UserInfo` . b., `Title` usw.), um sicherzustellen, dass der Status der aktuellen Aktivität weiterhin angezeigt wird. Wenn das System die- `UserActivityWillSave` Methode aufruft, `NeedsSave` wird das-Flag gelöscht. Wenn Sie die Daten Eigenschaften der Aktivität ändern, müssen Sie `NeedsSave` erneut auf festlegen `true` .

Anstatt die `UserActivityWillSave` oben dargestellte Methode zu verwenden, können Sie `UIKit` `AppKit` die Benutzeraktivität optional auch automatisch verwalten. Legen Sie hierzu die-Eigenschaft des Beantworter-Objekts fest, `UserActivity` und implementieren Sie die- `UpdateUserActivityState` Methode. Weitere Informationen finden Sie weiter unten [im Abschnitt unterstützende Handoff in Responders](#supporting-handoff-in-responders) .

### <a name="app-framework-support"></a>App Framework-Unterstützung

Sowohl `UIKit` (IOS) als auch `AppKit` (OS X) bieten integrierte Unterstützung für die Übergabe in den `NSDocument` Klassen, Responder ( `UIResponder` / `NSResponder` ) und `AppDelegate` . Obwohl jedes Betriebssystem die Übergabe etwas anders implementiert, sind der grundlegende Mechanismus und die APIs identisch.
...
Wenn dieser Schlüssel vorhanden ist, `NSDocument` Erstellen und `UIDocument` automatisch `NSUserActivity` Instanzen für icloud-basierte Dokumente des angegebenen Typs. Sie müssen für jeden Dokumenttyp, der von der App unterstützt wird, einen Aktivitätstyp bereitstellen, und mehrere Dokumenttypen können denselben Aktivitätstyp verwenden. `NSDocument`Und `UIDocument` füllen die `UserInfo` -Eigenschaft von automatisch mit dem `NSUserActivity` Wert Ihrer `FileURL` Eigenschaft auf.

Unter OS X werden die `NSUserActivity` von verwalteten `AppKit` und zugeordneten Responder automatisch zur aktuellen Aktivität, wenn das Fenster des Dokuments zum Hauptfenster wird. Unter IOS `NSUserActivity` müssen Sie für Objekte, `UIKit` die von verwaltet werden, entweder `BecomeCurrent` die-Methode explizit oder die-Eigenschaft des Dokuments `UserActivity` auf festlegen, `UIViewController` Wenn die app in den Vordergrund steht.

`AppKit`alle Eigenschaften, die `UserActivity` auf diese Weise auf OS X erstellt werden, werden von automatisch wieder hergestellt. Dies tritt auf, wenn die `ContinueUserActivity` Methode zurückgibt `false` oder wenn Sie nicht implementiert ist. In dieser Situation wird das Dokument mit der `OpenDocument` -Methode von geöffnet, `NSDocumentController` und es wird dann ein- `RestoreUserActivityState` Methoden Aufrufvorgang empfangen.
...
`UIKit`Und `AppKit` können eine Benutzeraktivität automatisch verwalten, wenn Sie Sie als-Eigenschaft eines Beantworter-Objekts festlegen `UserActivity` . Wenn der Zustand geändert wurde, müssen Sie die `NeedsSave` -Eigenschaft des-Responders `UserActivity` auf festlegen `true` . Das System speichert den ggf. automatisch `UserActivity` , nachdem er der Antwortzeit das Aktualisieren des Zustands durch Aufrufen seiner- `UpdateUserActivityState` Methode erteilt hat.

Wenn mehrere Antwort Empfänger eine einzelne Instanz gemeinsam nutzen `NSUserActivity` , erhalten Sie einen `UpdateUserActivityState` Rückruf, wenn das System das Benutzer Aktivitäts Objekt aktualisiert. Der-Beantworter muss die `AddUserInfoEntries` -Methode aufrufen, um `NSUserActivity` das `UserInfo` Wörterbuch zu aktualisieren, um den aktuellen Aktivitäts Zustand an diesem Punkt widerzuspiegeln. Das `UserInfo` Wörterbuch wird vor jedem- `UpdateUserActivityState` Rückruf gelöscht.

Um die Zuordnung zu einer Aktivität aufzuheben, kann ein Beantworter seine- `UserActivity` Eigenschaft auf festlegen `null` . Wenn eine verwaltete App Framework- `NSUserActivity` Instanz nicht mehr zugeordnete Responder oder Dokumente enthält, wird Sie automatisch für ungültig erklärt.
...
Der `AppDelegate` primäre Einstiegspunkt Ihrer APP ist der primäre Einstiegspunkt, wenn eine Übergabe Fortsetzung verarbeitet wird. Wenn der Benutzer auf eine Übergabe Benachrichtigung antwortet, wird die entsprechende App gestartet (wenn Sie nicht bereits ausgeführt wird), und die- `WillContinueUserActivityWithType` Methode des `AppDelegate` wird aufgerufen. An diesem Punkt sollte die APP den Benutzer darüber informieren, dass die Fortsetzung gestartet wird.

Die `NSUserActivity` -Instanz wird übermittelt, wenn die `AppDelegate` - `ContinueUserActivity` Methode des aufgerufen wird. An diesem Punkt sollten Sie die Benutzeroberfläche der App konfigurieren und die angegebene Aktivität fortsetzen.
...
3. Wenn Sie dies noch nicht getan haben, klicken Sie auf Bezeichner **, und erstellen** Sie eine ID für Ihre APP (z. b.), und bearbeiten Sie andernfalls `com.company.appname` Ihre vorhandene ID.
4. Stellen Sie sicher, dass der **icloud** -Dienst auf die angegebene ID geprüft wurde:

    [![Aktivieren Sie den icloud-Dienst für die angegebene ID.](handoff-images/provision01.png)](handoff-images/provision01.png#lightbox)
...
6. Klicken Sie auf **Bereitstellungs profile**  >  **Entwicklung** , und erstellen Sie ein neues Entwicklungs Bereitstellungs Profil für Ihre APP:
    ...
    [![Erstellen eines neuen Entwicklungs Bereitstellungs Profils für die APP](handoff-images/provision02.png)](handoff-images/provision02.png#lightbox)
...
8. Bearbeiten Sie die xamarin. IOS-Projektoptionen, und stellen Sie sicher, dass Sie das soeben erstellte Bereitstellungs Profil verwenden:
    ...
    [![Wählen Sie das soeben erstellte Bereitstellungs Profil aus.](handoff-images/provision03.png)](handoff-images/provision03.png#lightbox)
9. Bearbeiten Sie als nächstes die Datei " **Info. plist** ", und stellen Sie sicher, dass Sie die APP-ID verwenden, die zum Erstellen des Bereitstellungs Profils verwendet wurde:
    ...
    [![APP-ID festlegen](handoff-images/provision04.png)](handoff-images/provision04.png#lightbox)
10. Scrollen Sie zum Abschnitt **hintergrundmodi** , und überprüfen Sie die folgenden Elemente:

    [![Erforderliche hintergrundmodi aktivieren](handoff-images/provision05.png)](handoff-images/provision05.png#lightbox)
...
Benutzeraktivitäten können in apps fortgesetzt werden, die mit derselben Entwickler-Team-ID signiert sind und denselben Aktivitätstyp unterstützen. Zum Implementieren von "Handoff" in einer xamarin. IOS-App müssen Sie ein Benutzer Aktivitäts Objekt (entweder in `UIKit` oder `AppKit` ) erstellen, den Zustand des Objekts aktualisieren, um die Aktivität zu verfolgen, und die Aktivität auf einem empfangenden Gerät fortsetzen.
...
Der Aktivitätstyp Bezeichner wird verwendet, wenn eine-Instanz erstellt wird `NSUserActivity` , um den Typ der Aktivität zu identifizieren. Wenn eine Aktivität auf einem anderen Gerät fortgesetzt wird, bestimmt der Aktivitätstyp (zusammen mit der Team-ID der APP), welche App gestartet werden soll, um die Aktivität fortzusetzen.
...
Um die erforderlichen Aktivitätstypen Bezeichner zu erstellen, um dieses Verhalten zu unterstützen, bearbeiten Sie die Datei **Info. plist** , und wechseln Sie zur **Quell** Ansicht. Fügen Sie einen Schlüssel hinzu, `NSUserActivityTypes` und erstellen Sie die folgenden Bezeichner:

[![Der nsuseractivitytypes-Schlüssel und erforderliche Bezeichner im plist-Editor](handoff-images/type01.png)](handoff-images/type01.png#lightbox)
...
Wenn eine neue Instanz der `NSUserActivity` -Klasse erstellt wird, wird eine-Instanz angegeben, `NSUserActivityDelegate` um Änderungen am Zustand der Aktivität zu verfolgen. Der folgende Code kann z. b. zum Nachverfolgen von Zustandsänderungen verwendet werden:
...
Die `UserActivityWillSave` -Methode wird aufgerufen, bevor Änderungen an der Aktivität gespeichert und über lokal verfügbare Geräte synchronisiert werden. Sie können diese Methode verwenden, um Änderungen an der- `UserInfo` Eigenschaft der-Instanz vorzunehmen, `NSUserActivity` bevor Sie gesendet wird.

### <a name="creating-a-nsuseractivity-instance"></a>Erstellen einer nsuseractivity-Instanz

Jede Aktivität, die Ihre APP bereitstellen möchte, um auf einem anderen Gerät fortzufahren, muss in einer-Instanz gekapselt werden `NSUserActivity` . Die APP kann beliebig viele Aktivitäten erstellen, und die Art dieser Aktivitäten hängt von der Funktionalität und den Features der betreffenden App ab. Eine e-Mail-app könnte z. b. eine Aktivität zum Erstellen einer neuen Nachricht und eine weitere zum Lesen einer Nachricht erstellen.

Bei der Beispiel-APP `NSUserActivity` wird jedes Mal ein neues erstellt, wenn der Benutzer eine neue URL in einer der Webbrowser Ansicht im Registerkarten Format eingibt. Der folgende Code speichert den Status einer bestimmten Registerkarte:
...
Mit einem der `NSUserActivity` oben erstellten benutzeraktivitätstyp wird ein neuer erstellt, und es wird ein lesbarer Titel für die Aktivität bereitstellt. Sie wird an eine Instanz von angefügt, `NSUserActivityDelegate` um Zustandsänderungen zu überwachen, und informiert IOS, dass diese Benutzeraktivität die aktuelle Aktivität ist.

### <a name="populating-the-userinfo-dictionary"></a>Auffüllen des userinfo-Wörterbuchs

Wie bereits erwähnt, ist die- `UserInfo` Eigenschaft der- `NSUserActivity` Klasse eine `NSDictionary` von Schlüssel-Wert-Paaren, die verwendet wird, um den Status einer bestimmten Aktivität zu definieren. Bei den in gespeicherten Werten muss es sich um `UserInfo` einen der folgenden Typen handeln: `NSArray` , `NSData` , `NSDate` , `NSDictionary` , `NSNull` , `NSNumber` , `NSSet` , `NSString` oder `NSURL` . `NSURL`Datenwerte, die auf icloud-Dokumente verweisen, werden automatisch so angepasst, dass Sie auf die gleichen Dokumente auf einem empfangenden Gerät zeigen.
...
Bei der Übergabe werden automatisch lokale IOS-und OS X-Geräte, die sich in physischer Nähe zum ursprünglichen Gerät befinden und bei demselben icloud-Konto angemeldet sind, von der Verfügbarkeit von Fort Setz baren Benutzeraktivitäten informiert. Wenn sich der Benutzer entscheidet, eine Aktivität auf einem neuen Gerät fortzusetzen, startet das System die entsprechende app (basierend auf der Team-ID und dem Aktivitätstyp) und die Informationen `AppDelegate` , die für die Fortsetzung benötigt werden.
...
Der `ContinueUserActivity` von `AppDelegate` wird aufgerufen, um die angegebene Aktivität tatsächlich fortzusetzen. Auch in unserer Beispiel-App:
...
Die- `ContinueUserActivity` Methode enthält ein `UIApplicationRestorationHandler` , das Sie für das Fortsetzen von Dokumenten oder Beantworter-Aktivität aufrufen können. Wenn Sie aufgerufen werden, müssen Sie ein `NSArray` oder wiederherstellbare Objekte an den Wiederherstellungs Handler übergeben. Beispiel:

```csharp
completionHandler (new NSObject[]{Tab4});
```

Für jedes Objekt, das erfolgreich ist, wird die zugehörige- `RestoreUserActivityState` Methode aufgerufen. Jedes-Objekt kann dann die Daten im `UserInfo` Wörterbuch verwenden, um seinen eigenen Zustand wiederherzustellen. Beispiel:
...
Für Dokument basierte apps: Wenn Sie die Methode nicht implementieren `ContinueUserActivity` oder zurückgibt `false` , `UIKit` `AppKit` kann die Aktivität automatisch fortgesetzt werden. Weitere Informationen finden Sie weiter unten [im Abschnitt unterstützende Übergabe von Dokument basierten apps](#supporting-handoff-in-document-based-apps) .
...
Wenn ein Fehler auftritt, wird die- `DidFailToContinueUserActivitiy` Methode von `AppDelegate` aufgerufen. Beispiel:
...
Verwenden Sie die bereitgestellte `NSError` , um dem Benutzerinformationen über den Fehler bereitzustellen.
...
Wenn die ursprüngliche, systemeigene APP die URL für die Weboberfläche kennt (und die erforderliche Syntax zum Identifizieren des angegebenen Elements ist), kann diese Informationen in der-Eigenschaft der-Instanz codiert werden `WebpageURL` `NSUserActivity` . Wenn auf dem empfangenden Gerät keine geeignete Native App installiert ist, um die Fortsetzung zu verarbeiten, kann die bereitgestellte Weboberfläche aufgerufen werden.

## <a name="web-browser-to-native-app-handoff"></a>Webbrowser zu nativer App-Übergabe

Wenn der Benutzer eine webbasierte Schnittstelle auf dem Ursprungs Gerät verwendet und eine native App auf dem empfangenden Gerät den Domänen Teil der `WebpageURL` Eigenschaft beansprucht, verwendet das System diese APP zum Behandeln der Fortsetzung. Das neue Gerät empfängt eine- `NSUserActivity` Instanz, die den Aktivitätstyp als markiert `BrowsingWeb` `WebpageURL` , und enthält die URL, die der Benutzer besucht hat `UserInfo` . das Wörterbuch ist leer.
...
Wenn die angegebene Domäne mit dem `WebpageURL` Wert einer Eigenschaft übereinstimmt, wird von Handoff eine Liste der genehmigten App-IDs von der Website in dieser Domäne heruntergeladen. Die Website muss eine Liste genehmigter IDs in einer signierten JSON-Datei mit dem Namen **Apple-App-Site-Association** (z `https://company.com/apple-app-site-association` . b.) bereitstellen.

Diese JSON-Datei enthält ein Wörterbuch, das eine Liste der APP-IDs im Formular angibt `<team identifier>.<bundle identifier>` . Beispiel:
...
Um die JSON-Datei zu signieren (sodass Sie die richtige ist `Content-Type` `application/pkcs7-mime` ), verwenden Sie die **Terminal** -APP und einen `openssl` Befehl mit einem Zertifikat und Schlüssel, das von einer Zertifizierungsstelle ausgestellt wurde, die von IOS als vertrauenswürdig eingestuft wird (eine Liste finden Sie unter [https://support.apple.com/kb/ht5012](https://support.apple.com/kb/ht5012) ). Beispiel:
...
Die App empfängt eine beliebige Aktivität, deren `WebpageURL` Domäne Ihre `com.apple.developer.associated-domains` Berechtigung hat. Nur die `http` `https` Protokolle und werden unterstützt, jedes andere Protokoll gibt eine Ausnahme aus.

## <a name="supporting-handoff-in-document-based-apps"></a>Unterstützende Übergabe in Dokument basierten apps

Wie oben bereits erwähnt, unterstützen Dokument basierte apps unter IOS und OS X automatisch die Übergabe von icloud-basierten Dokumenten, wenn die Datei " **Info. plist** " der APP einen `CFBundleDocumentTypes` Schlüssel von enthält `NSUbiquitousDocumentUserActivityType` . Beispiel:
...
Auf das automatisch erstellte Benutzer Aktivitäts Objekt (das über die-Eigenschaft des Dokuments verfügbar ist `UserActivity` ) kann von anderen Objekten in der APP verwiesen und zum Wiederherstellen des Zustands bei der Fortsetzung verwendet werden. Beispielsweise, um die Elementauswahl und die Dokument Position zu verfolgen. Sie müssen diese Aktivitäts `NeedsSave` Eigenschaft auf festlegen, `true` Wenn sich der Status ändert, und das `UserInfo` Wörterbuch in der- `UpdateUserActivityState` Methode aktualisieren.
...
Die ursprüngliche App legt die- `SupportsContinuationStreams` Eigenschaft der- `NSUserActivity` Instanz auf fest `true` . Beispiel:
...
Die empfangende App kann dann die-Methode von in der-Klasse aufzurufen `GetContinuationStreams` `NSUserActivity` `AppDelegate` , um den Stream einzurichten. Beispiel:
...
Sie verwenden ein-Objekt, `NSInputStream` um schreibgeschützten Zugriff auf Streamdaten bereitzustellen, und ein `NSOutputStream` schreibgeschützten Zugriff. Die Streams sollten in Anforderungs-und Antwort Weise verwendet werden, wobei die empfangende app mehr Daten anfordert und die ursprüngliche app Sie bereitstellt. Das heißt, dass Daten, die in den Ausgabestream auf dem ursprünglichen Gerät geschrieben werden, aus dem Eingabestream auf dem fortlaufenden Gerät gelesen werden und umgekehrt.
...
- Verwenden Sie beim Angeben der Aktivitätstypen Reverse-DNS-Notation, um Konflikte zu vermeiden. Wenn eine Aktivität für eine bestimmte App spezifisch ist, sollte Ihr Name in die Typdefinition eingeschlossen werden (z `com.myCompany.myEditor.editing` . b.). Wenn die Aktivität über mehrere apps hinweg funktionieren kann, löschen Sie den Namen der APP aus der Definition (z `com.myCompany.editing` . b.).
- Wenn Ihre APP den Status einer Benutzeraktivität aktualisieren muss ( `NSUserActivity` ), legen Sie die- `NeedsSave` Eigenschaft auf fest `true` . Zu den entsprechenden Zeitpunkten wird die-Methode des Delegaten aufgerufen, `UserActivityWillSave` sodass Sie das `UserInfo` Wörterbuch nach Bedarf aktualisieren können.
- Da der Übergabeprozess auf dem empfangenden Gerät nicht sofort initialisiert werden kann, sollten Sie das `AppDelegate` implementieren `WillContinueUserActivity` und den Benutzer darüber informieren, dass eine Fortsetzung gestartet wird.

## <a name="example-handoff-app"></a>Beispiel für eine Handoff-App

Als Beispiel für die Verwendung von Handoff in einer xamarin. IOS-APP haben wir die Beispiel-app " [**monkeybrowser**](https://docs.microsoft.com/samples/xamarin/ios-samples/ios8-monkeybrowser) " in dieses Handbuch integriert. Die APP verfügt über vier Registerkarten, die der Benutzer zum Durchsuchen des Internets verwenden kann, die jeweils einen bestimmten Aktivitätstyp haben: weather, Favorit, Coffee Break und work.

Wenn der Benutzer auf jeder Registerkarte eine neue URL eingibt und auf die Schaltfläche " **go** " tippt, `NSUserActivity` wird für die Registerkarte eine neue erstellt, die die URL enthält, die der Benutzer gerade durchsucht:

[![Beispiel für eine Handoff-App](handoff-images/handoff01.png)](handoff-images/handoff01.png#lightbox)

Wenn auf einem der Geräte des Benutzers die **monkeybrowser** -App installiert ist, mit demselben Benutzerkonto bei icloud angemeldet ist, sich im selben Netzwerk befindet und sich in der Nähe des obigen Geräts befindet, wird die Handoff-Aktivität auf dem Startbildschirm (in der unteren linken Ecke) angezeigt:

[![Die Handoff-Aktivität, die auf dem Startbildschirm in der unteren linken Ecke angezeigt wird.](handoff-images/handoff02.png)](handoff-images/handoff02.png#lightbox)

Wenn der Benutzer das Handoff-Symbol nach oben zieht, wird die APP gestartet, und die in der angegebene Benutzeraktivität wird `NSUserActivity` auf dem neuen Gerät fortgesetzt:

[![Die Benutzeraktivität wurde auf dem neuen Gerät fortgesetzt.](handoff-images/handoff03.png)](handoff-images/handoff03.png#lightbox)

Wenn die Benutzeraktivität erfolgreich an ein anderes Apple-Gerät gesendet wurde, empfängt das sendende Gerät `NSUserActivity` einen Rückruf an die-Methode in der, `UserActivityWasContinued` `NSUserActivityDelegate` um Sie darüber zu informieren, dass die Benutzeraktivität erfolgreich auf ein anderes Gerät übertragen wurde.
