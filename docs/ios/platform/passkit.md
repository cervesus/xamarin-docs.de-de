...
Dieses Dokument enthält eine Einführung in Wallet und die Verwendung der passkit-API mit xamarin. IOS.
 ...
 [![Die Geldbörse speichert und organisiert alle Pass-ins auf einem Telefon.](passkit-images/image1.png)](passkit-images/image1.png#lightbox)
...
Passkit ist nicht nur eine API in cocoatouch, sondern gehört zu einem größeren Ökosystem von apps, Daten und Diensten, die das sichere freigeben und Verwalten von Barcodes und anderen Daten erleichtern. Dieses allgemeine Diagramm zeigt die verschiedenen Entitäten, die an der Erstellung und Verwendung von Durchläufen beteiligt sein können:
 ...
 [![Dieses allgemeine Diagramm zeigt die Entitäten, die beim Erstellen und Verwenden von Durchläufen beteiligt sind.](passkit-images/image2.png)](passkit-images/image2.png#lightbox)
...
Die fünf Pass Typen sind in diesem Screenshot dargestellt (in der folgenden Reihenfolge: Coupon, generisch, Store Card, Boarding Pass und Ereignis Ticket):
 ...
 [![Die fünf Pass Typen sind in diesem Screenshot dargestellt.](passkit-images/image3.png)](passkit-images/image3.png#lightbox)
...
- **pass.jsauf** – erforderlich. Enthält alle Informationen für den Durchlauf.
- **manifest.jsauf** – erforderlich. Enthält SHA1-Hashes für jede Datei im Durchlauf außer der Signatur Datei und dieser Datei (manifest.jsauf).
- **Signatur** – erforderlich. Wird erstellt, indem die `manifest.json` Datei mit dem Zertifikat signiert wird, das im IOS-Bereitstellungs Portal generiert wurde.
- **logo.png** – optional.
...
Die Verzeichnisstruktur einer Pass Datei ist unten dargestellt (Dies ist der Inhalt des ZIP-Archivs):
 ...
 [![Hier wird die Verzeichnisstruktur einer Pass Datei angezeigt.](passkit-images/image4.png)](passkit-images/image4.png#lightbox)
...
- **serialNumber** – eine eindeutige Zeichenfolge in diesem `passTypeidentifier` . Der Wert ist für Wallet nicht transparent, aber wichtig für die Nachverfolgung spezifischer Durchgänge bei der Kommunikation mit dem Server.
...
Das Übersetzen eines Pass-Through in mehrere Sprachen ähnelt dem Lokalisieren einer IOS-Anwendung – erstellen Sie sprachspezifische Verzeichnisse mit der `.lproj` Erweiterung, und platzieren Sie die lokalisierten Elemente in. Text Übersetzungen sollten in eine Datei eingegeben werden `pass.strings` , während lokalisierte Bilder den gleichen Namen wie das Image aufweisen, das Sie im Durchlauf Stamm ersetzen.
...
1. Berechnen Sie einen SHA1-Hash für jede Datei im Pass-Verzeichnis (schließen Sie `manifest.json` weder die-Datei noch die- `signature` Datei ein, die in dieser Phase überhaupt nicht vorhanden sein sollte).
1. Schreiben Sie `manifest.json` als JSON-Schlüssel-Wert-Liste jedes Datei namens mit dem Hashwert.
1. Verwenden Sie das Zertifikat, um die Datei zu signieren, `manifest.json` und schreiben Sie das Ergebnis in eine Datei namens `signature` .
1. Packen Sie alles nach oben, und übergeben Sie der resultierenden Datei eine `.pkpass` Dateierweiterung.
...
1. Navigieren Sie im [IOS-Bereitstellungs Portal im Abschnitt Zertifikate, Bezeichner und Profile](https://developer.apple.com/account/overview.action)zu **Identifizierer** , und wählen Sie **Passtyp-IDs** aus. Wählen Sie dann die **+** Schaltfläche aus, um einen neuen Passtyp zu erstellen: [ ![ neuen Passtyp erstellen](passkit-images/passid.png)](passkit-images/passid.png#lightbox)

2. Geben Sie eine **Beschreibung** (Name) und einen **Bezeichner** (eindeutige Zeichenfolge) für den Durchlauf an. Beachten Sie, dass alle Pass-Type-IDs mit der Zeichenfolge beginnen müssen, `pass.` wie in diesem Beispiel `pass.com.xamarin.coupon.banana` : [ ![ Geben Sie eine Beschreibung und einen Bezeichner](passkit-images/register.png)](passkit-images/register.png#lightbox)
...
1. Wählen Sie die neu erstellte Pass-ID aus der Liste aus, und klicken Sie auf **Bearbeiten** : [ ![ Wählen Sie die neue Pass-ID aus der Liste aus](passkit-images/pass-done.png)](passkit-images/pass-done.png#lightbox)

    Klicken Sie dann auf **Zertifikat erstellen...** :

    [![Wählen Sie Zertifikat erstellen.](passkit-images/cert-dist.png)](passkit-images/cert-dist.png#lightbox)
...
- Erstellen Sie eine pass.jsfür die Datei, die alle erforderlichen Daten enthält.
- Schließt Bilder in den Ordner ein (falls erforderlich).
- Berechnen Sie SHA1-Hashes für jede Datei im Ordner, und schreiben Sie in manifest.js.
- Melden Sie sich mit der heruntergeladenen Datei "Certificate. p12" an manifest.js.
...
Im [Beispielcode](https://docs.microsoft.com/samples/xamarin/ios-samples/passkit) für diesen Artikel sind einige Quelldateien vorhanden, die verwendet werden können, um einen Durchlauf zu generieren. Verwenden Sie die Dateien im `CouponBanana.raw` Verzeichnis des Verzeichnisses "kreateapassmanual". Die folgenden Dateien sind vorhanden:
 ...
 [![Diese Dateien sind vorhanden.](passkit-images/image18.png)](passkit-images/image18.png#lightbox)

Öffnen Sie pass.jsauf, und bearbeiten Sie die JSON-Datei. Sie müssen mindestens den `passTypeIdentifier` und `teamIdentifer` entsprechend Ihrem Apple-Entwicklerkonto aktualisieren.
...
Laden Sie die **Support Materialien für Wallet-Seed** von der Apple- [Download](https://developer.apple.com/downloads/index.action?name=Passbook) Website herunter. Verwenden `signpass` Sie das Tool, um den Ordner in einen Durchlauf umzuwandeln (Dadurch werden auch die SHA1-Hashwerte berechnet und die Ausgabe in eine pkpass-Datei gezippt).

#### <a name="testing"></a>Testen

Wenn Sie die Ausgabe dieser Tools untersuchen (indem Sie den Dateinamen auf. zip festlegen und ihn dann öffnen), werden die folgenden Dateien angezeigt (Beachten Sie das Hinzufügen der `manifest.json` `signature` Dateien und):

 [![Überprüfen der Ausgabe dieser Tools](passkit-images/image19.png)](passkit-images/image19.png#lightbox)
...
Nachdem Sie die Datei signiert, gezippt und umbenannt haben (z. b. zu `BananaCoupon.pkpass` ) Sie können Sie in den Simulator ziehen, um Sie zu testen, oder per e-Mail an sich selbst, um Sie auf einem echten Gerät abzurufen. Sie sollten einen Bildschirm sehen, um den Durchlauf **hinzuzufügen** , wie folgt:
 ...
 [![Hinzufügen des Pass-Bildschirms](passkit-images/image20.png)](passkit-images/image20.png#lightbox)
...
Wallet ist der zentrale Bestandteil des passkit-Ökosystems. Dieser Screenshot zeigt die leere Brieftasche und die Art und Weise, wie die Pass Liste und die einzelnen Durchgänge aussehen:
 ...
 [![Dieser Screenshot zeigt die leere Brieftasche und die Art und Weise, wie die Pass Liste und die einzelnen Durchgänge aussehen](passkit-images/image21.png)](passkit-images/image21.png#lightbox)
...
- **Begleit Anwendungen** – diese werden von Anbietern geschrieben, um Pass-und-Funktionen zu verteilen und zusätzliche Funktionen zum Durchsuchen oder bearbeiten zu bieten. Xamarin. IOS-Anwendungen verfügen über vollständigen Zugriff auf die passkit-API zum Erstellen und Bearbeiten von Durchläufen. Durch Pass können dann mithilfe von der Wallet hinzugefügt werden `PKAddPassesViewController` . Dieser Vorgang wird im Abschnitt " **begleitende Anwendungen** " dieses Dokuments ausführlicher beschrieben.
...
Dieser Screenshot zeigt, wie **e-Mail** in ios 6 eine Pass-Anlage erkennt und (wenn Sie berührt) Angebote zum **Hinzufügen zur Wallet hinzufügen** können.
 ...
 [![Dieser Screenshot zeigt, wie e-Mail in ios 6 eine Pass Anlage erkennt.](passkit-images/image22.png)](passkit-images/image22.png#lightbox)

 [![Dieser Screenshot zeigt, wie e-Mail Angebote zum Hinzufügen einer Pass Anlage zu Wallet anbietet.](passkit-images/image23.png)](passkit-images/image23.png#lightbox)
...
Der grundlegende Vorgang einer Kanal Anwendung besteht darin, die Pass-Datei abzurufen und passkit aufzurufen `PKAddPassesViewController` , um dem Benutzer die Möglichkeit zu geben, den Pass der Geldbörse hinzuzufügen. Die Implementierung dieses Ansichts Controllers wird im nächsten Abschnitt zu begleitenden **Anwendungen**behandelt.
...
### <a name="entitlements"></a>Ansprüche
...
![Berechtigungen. plst-Editor](passkit-images/image31.png)

Wählen Sie im Abschnitt Wallet die Option **Wallet aktivieren** aus.

![Wallet-Berechtigung aktivieren](passkit-images/image32.png)
...
Um die Wallet-Berechtigung hinzuzufügen, legen Sie die- **Eigenschaft** `Passbook Identifiers` in der Dropdown Liste auf fest, wodurch der **Typ** automatisch festgelegt wird `Array` . Legen Sie dann den Zeichen folgen **Wert** auf fest `$(TeamIdentifierPrefix)*` :

![Wallet-Berechtigung aktivieren](passkit-images/image33.png)

So akzeptiert Ihre App alle Passtypen. Um Ihre App einzuschränken und nur eine Teilmenge von Teampasstypen zu erlauben, legen Sie den Zeichenfolgenwert auf  fest.

`$(TeamIdentifierPrefix)pass.$(CFBundleIdentifier)`

Dabei `pass.$(CFBundleIdentifier)` ist die [oben](~/ios/platform/passkit.md) erstellte Pass-ID.

-----

### <a name="debugging"></a>Debuggen

Wenn bei der Bereitstellung der Anwendung Probleme auftreten, überprüfen Sie, ob Sie das richtige **Bereitstellungs Profil** verwenden und ob `Entitlements.plist` in den **iPhone Bundle-Signierungs** Optionen als **benutzerdefinierte** Berechtigungsdatei ausgewählt ist.
...
dann ist das Berechtigungs `pass-type-identifiers` Array falsch (oder stimmt nicht mit dem **Bereitstellungs Profil**). Überprüfen Sie, ob die Passtyp-IDs und Ihre Team-ID korrekt sind.
...
Beachten Sie, dass der Simulator die Liste der zurückgegebenen Pässe nicht filtert, daher sollte diese Methode immer auf echten Geräten getestet werden. Diese Liste kann in einer uitableview angezeigt werden. Die [Beispiel-App](https://docs.microsoft.com/samples/xamarin/ios-samples/passkit) sieht wie folgt aus, nachdem zwei Coupons hinzugefügt wurden:
 ...
 [![Die Beispiel-App sieht wie folgt aus, nachdem zwei Coupons hinzugefügt wurden.](passkit-images/image29.png)](passkit-images/image29.png#lightbox)
...
Diese Zeichenfolge wird im [Beispiel](https://docs.microsoft.com/samples/xamarin/ios-samples/passkit)als Warnung angezeigt:
 ...
 [![Die von Coupon ausgewählte Warnung im Beispiel](passkit-images/image30.png)](passkit-images/image30.png#lightbox)
...
Der Durchlauf wird mit den Optionen **Hinzufügen** und **Abbrechen** angezeigt:
 ...
 [![Die mit Add-und Cancel-Optionen vorgestellte Übergabe](passkit-images/image20.png)](passkit-images/image20.png#lightbox)
...
Wie bereits erwähnt, kann nur Wallet einen Durchlauf zum Scannen anzeigen. Ein Durchlauf kann mithilfe der-Methode wie folgt angezeigt werden `OpenUrl` :

 `UIApplication.SharedApplication.OpenUrl (p.PassUrl);`

### <a name="receiving-notifications-of-changes"></a>Empfangen von Benachrichtigungen über Änderungen

Anwendungen können mithilfe von Änderungen an der Pass Bibliothek überwachen `PKPassLibraryDidChangeNotification` . Änderungen können dadurch verursacht werden, dass Benachrichtigungen im Hintergrund aktualisiert werden. Daher empfiehlt es sich, Sie in Ihrer APP zu überwachen.
...
Weitere Informationen finden [Sie unter dotnet-Passbook](https://github.com/tomasmcguinness/dotnet-passbook) Open Source-serverseitiger c#-Code.
