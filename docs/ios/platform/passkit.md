...
1. Berechnen Sie einen SHA1-Hash für jede Datei im Pass-Verzeichnis (schließen Sie nicht die `manifest.json` oder `signature` Datei ein, die in dieser Phase überhaupt nicht vorhanden sein sollten).
1. Schreiben Sie `manifest.json` als JSON-Schlüssel-Wert-Liste jedes Datei namens mit dem Hashwert.
1. Verwenden Sie das Zertifikat zum Signieren der `manifest.json` Datei, und schreiben Sie das Ergebnis in eine Datei mit dem Namen `signature`.
...
2. Geben Sie eine **Beschreibung** (Name) und einen **Bezeichner** (eindeutige Zeichenfolge) für den Durchlauf an. Beachten Sie, dass alle Passtyp-IDs mit der Zeichenfolge beginnen müssen, `pass.` in diesem Beispiel `pass.com.xamarin.coupon.banana` verwendet wird:[![](passkit-images/register.png "Angeben einer Beschreibung und eines Bezeichners")](passkit-images/register.png#lightbox)
...
Öffnen Sie "Pass. JSON", und bearbeiten Sie die JSON. Sie müssen zumindest die `passTypeIdentifier` und `teamIdentifer` aktualisieren, damit Sie Ihrem Apple-Entwicklerkonto entsprechen.
...
Laden Sie die **Support Materialien für Wallet-Seed** von der Apple- [Download](https://developer.apple.com/downloads/index.action?name=Passbook) Website herunter. Verwenden Sie das `signpass` Tool, um den Ordner in einen Durchlauf umzuwandeln (Dadurch werden auch die SHA1-Hashwerte berechnet und die Ausgabe in eine pkpass-Datei gezippt).

#### <a name="testing"></a>Test

Wenn Sie die Ausgabe dieser Tools untersuchen (indem Sie den Dateinamen auf. zip festlegen und ihn dann öffnen), werden die folgenden Dateien angezeigt (Beachten Sie das Hinzufügen der `manifest.json`-und `signature` Dateien):
...
- **Begleit Anwendungen** – diese werden von Anbietern geschrieben, um Pass-und-Funktionen zu verteilen und zusätzliche Funktionen zum Durchsuchen oder bearbeiten zu bieten. Xamarin. IOS-Anwendungen verfügen über vollständigen Zugriff auf die passkit-API zum Erstellen und Bearbeiten von Durchläufen. Mit dem `PKAddPassesViewController`können dann Pass-ins hinzugefügt werden. Dieser Vorgang wird im Abschnitt " **begleitende Anwendungen** " dieses Dokuments ausführlicher beschrieben.
...
Der grundlegende Vorgang einer Kanal Anwendung besteht darin, die Pass-file-Datei abzurufen und die `PKAddPassesViewController` passkit aufzurufen, um dem Benutzer die Möglichkeit zu geben, den Pass der Geldbörse hinzuzufügen. Die Implementierung dieses Ansichts Controllers wird im nächsten Abschnitt zu begleitenden **Anwendungen**behandelt.
...
Um die Wallet-Berechtigung hinzuzufügen, legen Sie die- **Eigenschaft** in der Dropdown Liste auf `Passbook Identifiers` fest, wodurch der **Typ** `Array`automatisch festgelegt wird. Legen Sie dann den Zeichen folgen **Wert** auf `$(TeamIdentifierPrefix)*`fest:
...
Dabei ist `pass.$(CFBundleIdentifier)` die [oben](~/ios/platform/passkit.md) erstellte Pass-ID.

-----

### <a name="debugging"></a>Debugging

Wenn bei der Bereitstellung der Anwendung Probleme auftreten, überprüfen Sie, ob Sie das richtige **Bereitstellungs Profil** verwenden und dass das `Entitlements.plist` in den **iPhone Bundle-Signierungs** Optionen als **benutzerdefinierte** Berechtigungsdatei ausgewählt ist.
...
Das Array `pass-type-identifiers` Berechtigungen ist falsch (oder entspricht nicht dem **Bereitstellungs Profil**). Überprüfen Sie, ob die Passtyp-IDs und Ihre Team-ID korrekt sind.
...
Anwendungen können mithilfe der `PKPassLibraryDidChangeNotification`auf Änderungen an der Pass Bibliothek lauschen. Änderungen können dadurch verursacht werden, dass Benachrichtigungen im Hintergrund aktualisiert werden. Daher empfiehlt es sich, Sie in Ihrer APP zu überwachen.
