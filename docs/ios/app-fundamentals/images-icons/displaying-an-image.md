...
> [!IMPORTANT]
> **Hinweis**: Wenn zugewiesen, da ein Bild Bilder nicht richtig angezeigt werden, stellen Sie sicher, dass der richtige Dateiname verwendet wird die `FromBundle` Methode (die **Bildersatz** und nicht das übergeordnete Element **Asset-Katalog**  Name). PNG-Bilder die `.png` Erweiterung kann ausgelassen werden. Für andere Bildformate ist die Erweiterung erforderlich (z. b. `PurpleMonkey.jpg`) angezeigt wird.
