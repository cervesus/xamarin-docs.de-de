...
Vor IOS 8 hat der Entwickler `UIInterfaceOrientation` und `UIInterfaceIdiom` verwendet, um zwischen dem hoch-und Querformat sowie zwischen iPhone-und iPad-Geräten zu unterscheiden. In iOS8 werden Ausrichtung und Gerät mithilfe von *Größenklassen*bestimmt.
...
- **Regulär** – Dies ist entweder für eine große Bildschirmgröße (z. b. ein iPad) oder für eine Mini Anwendung, die einen großen Eindruck ergibt (z. b. ein `UIScrollView`
...
Das Bild Ladeverhalten in ios erkennt auch ein `@3x` Suffix für Bilddateien. Beispiel: Wenn der Entwickler ein Image-Asset (bei unterschiedlichen Auflösungen) im Paket der Anwendung mit den folgenden Dateinamen enthält: `MonkeyIcon.png`, `MonkeyIcon@2x.png`und `MonkeyIcon@3x.png`. Auf iPhone 6 und wird das `MonkeyIcon@3x.png` Abbild automatisch verwendet, wenn der Entwickler ein Image mit dem folgenden Code lädt:

```csharp
UIImage icon = UIImage.FromFile("MonkeyImage.png");
```

Wenn Sie das Abbild einem Benutzeroberflächen Element mithilfe des IOS-Designers als `MonkeyIcon.png`zuweisen, wird der `MonkeyIcon@3x.png` auf dem iPhone 6 Plus ebenfalls automatisch verwendet.
...
Merkmale sind Eigenschaften, die verwendet werden können, um zu bestimmen, wie sich ein Layout ändert, wenn sich die Umgebung ändert. Sie bestehen aus einem Satz von Eigenschaften (der `HorizontalSizeClass` und `VerticalSizeClass` basierend auf `UIUserInterfaceSizeClass`) sowie der Schnittstellen Ausdrucks Fläche (`UIUserInterfaceIdiom`) und der Anzeige Skala.
...
Der Entwickler erhält auch eine teilweise qualifizierte Merkmals Auflistung, wenn er eine der von Apple bereitgestellten Erstellungs Methoden (z. b. `UITraitCollection.FromHorizontalSizeClass`) verwendet, um eine neue Sammlung zu erstellen.
...
Um zwei Merkmale zu testen, verwenden Sie die `Contains`-Methode des `UITraitCollection` der den Wert des zu testenden Merkmals übergibt.

Der Entwickler kann die Vergleiche manuell im Code durchführen, um zu bestimmen, wie Sichten oder Ansichts Controller angezeigt werden. Allerdings verwendet `UIKit` intern diese Methode, um einige Funktionen bereitzustellen, wie z. b. den Proxy für die Darstellung.
...
Im Allgemeinen übergibt der Entwickler eine partiell angegebene Merkmals Auflistung an die `AppearanceForTraitCollection` Methode, z. b. eine Klasse, die nur eine horizontale Größenklasse von Compact angegeben hat, sodass Sie jede Ansicht in der Anwendung anpassen können, die horizontal komprimiert ist.

## <a name="uiimage"></a>Uiimage

Eine andere Klasse, der Apple eine Merkmals Auflistung hinzugefügt hat, ist `UIImage`. In der Vergangenheit musste der Entwickler eine @1X und @2x Version eines beliebigen bitzugeordneten Grafik Objekts angeben, das in der Anwendung enthalten sein soll (z. b. ein Symbol).
...
Ein `UIView` wird als übergeordnetes Element des Split View Controller festgelegt, und die `SetOverrideTraitCollection`-Methode wird für die Ansicht aufgerufen, die eine neue Merkmals Auflistung übergibt und den Split View Controller als Ziel verwendet. Die neue Merkmals Auflistung überschreibt die `HorizontalSizeClass`, indem Sie auf `Regular`festgelegt wird, sodass der Split View Controller sowohl die Master-als auch die Detailansicht auf einem iPhone in der Querformat Ansicht anzeigt.

Beachten Sie, dass die `VerticalSizeClass` auf `unspecified`festgelegt wurde, sodass die neue Merkmals Auflistung der Merkmals Auflistung auf dem übergeordneten Element hinzugefügt werden kann, was zu einer `Compact VerticalSizeClass` für den untergeordneten Split View Controller führt.
...
- Definieren Sie zunächst den neuen primären Ansichts Controller. Standardmäßig verwendet der Split View Controller automatisch den primären Ansichts Controller aus der reduzierten Ansicht. Der Entwickler kann dieses Verhalten überschreiben, indem er die `GetPrimaryViewControllerForExpandingSplitViewController`-Methode des `UISplitViewControllerDelegate` verwendet.
- Nachdem der primäre Ansichts Controller ausgewählt wurde, muss der sekundäre Ansichts Controller neu erstellt werden. Der Split View Controller schließt die automatische Verarbeitung dieser Phase basierend auf dem Hardware Gerät ein. Der Entwickler kann dieses Verhalten überschreiben, indem er die `SeparateSecondaryViewController`-Methode des `UISplitViewControllerDelegate` aufgerufen.

In einem Split View Controller spielt der primäre Ansichts Controller eine Rolle beim Erweitern und reduzieren der Sichten, indem er die `CollapseSecondViewController`-und `SeparateSecondaryViewController` Methoden der `UISplitViewControllerDelegate`implementiert. `UINavigationController` implementiert diese Methoden, um den sekundären Ansichts Controller automatisch per Push zu übermitteln.
...
Entwickler können `ShowViewController` und `ShowDetailViewController` in ihren eigenen benutzerdefinierten Ansichts Controllern implementieren, um die gleichen automatisierten Funktionen zu erhalten, die `UINavigationController` und `UISplitViewController` bereitstellt.

### <a name="how-it-works"></a>Funktionsweise

In diesem Abschnitt wird erläutert, wie diese Methoden tatsächlich in ios 8 implementiert werden. Sehen wir uns zunächst die neue `GetTargetForAction`-Methode an:
...
1. Wenn stattdessen eine `ShowDetailViewController`-Methode aufgerufen wurde, ist der Split View Controller der erste Ansichts Controller, um ihn zu implementieren, sodass er als übergeordnetes Element verwendet wird.

Die `GetTargetForAction`-Methode wird verwendet, indem ein Ansichts Controller gesucht wird, der eine bestimmte Aktion implementiert, und dann diesen Ansichts Controller anfordert, wenn diese Aktion empfangen werden soll. Da diese Methode öffentlich ist, können Entwickler ihre eigenen benutzerdefinierten Methoden erstellen, die genau wie die integrierten Methoden `ShowViewController` und `ShowDetailViewController` funktionieren.
...
Um die Änderungen innerhalb des Unified Storyboard-Systems zu berücksichtigen, wurde ein neues Controller Objekt erstellt, um die dargestellten Ansichts Controller – `UIPresentationController`zu verwalten. Dieser Controller wird ab dem Zeitpunkt der Darstellung des Ansichts Controllers erstellt, bis er verworfen wird. Da es sich um eine Verwaltungs Klasse handelt, kann Sie über den Ansichts Controller als eine übergeordnete Klasse angesehen werden, da Sie auf Geräteänderungen reagiert, die sich auf den Ansichts Controller auswirken (z. b. die Ausrichtung), die dann wieder in den Ansichts Controller der Präsentations Controller-Steuerelemente

Wenn der Entwickler einen Ansichts Controller mit der `PresentViewController`-Methode präsentiert, wird die Verwaltung des Präsentations Prozesses an `UIKit`übergeben. UIKit verarbeitet (unter anderem) den korrekten Controller für den Stil, der erstellt wird, wobei die einzige Ausnahme darin besteht, dass der Stil eines Ansichts Controllers auf `UIModalPresentationCustom`festgelegt ist. Hier kann die Anwendung ihren eigenen presentationcontroller bereitstellen, anstatt den `UIKit` Controller zu verwenden.
...
Dies wird erreicht, indem die `UpdateConstraintsForTraitCollection`-Methode des Ansichts Controllers überschrieben und die Einschränkungen basierend auf dem Wert des `VerticalSizeClass`angepasst werden. Beispiel:
...
Wenn der Split View Controller in der Adaptive Photos-Anwendung von reduziert zu erweitert wechselt, werden den Standard Animationen Animationen hinzugefügt, indem die `WillTransitionToTraitCollection`-Methode des Ansichts Controllers außer Kraft gesetzt wird. Beispiel:
...
Nun sehen wir uns an, wie das erweiternde und reduzierende Verhalten des Split View Controller in xamarin implementiert wurde. Wenn in der `AppDelegate`der Split View Controller erstellt wird, wird der zugehörige Delegat zugewiesen, um diese Änderungen zu behandeln:
...
Als nächstes sehen wir uns an, wie die Anwendung für Adaptive Fotos zwischen Ansichts Controllern verschoben wird. Wenn der Benutzer in der `AAPLConversationViewController`-Klasse eine Zelle aus der Tabelle auswählt, wird die `ShowDetailViewController`-Methode aufgerufen, um die Detailansicht anzuzeigen:
...
Diese werden mithilfe der `GetTargetViewControllerForAction`-Methode implementiert, die oben ausführlich erläutert wird.
...
Um diesen Effekt zu erzielen, klicken Sie im IOS-Designer-Steuerelement auf die Schaltfläche, und ziehen Sie eine Linie zum Ansichts Controller, die angezeigt werden soll. Wenn die Maustaste losgelassen wird, wählen Sie im Popup Menü segue-Typ die Option `Show Detail` aus:
...
Um einen Ausschluss Fall aus einem Element zu entfernen, wählen Sie einfach das Element im **Designoberfläche**aus, Scrollen Sie zum unteren Rand des **Eigenschaften-Explorers** , und klicken Sie neben dem Fall, den Sie entfernen möchten, auf die Schaltfläche **-** .
...
- Verwenden Sie eine einzelne Stamm Ansicht, bei der es sich um eine `UIView` oder `UIViewController` Objekt handelt.
- Stellen Sie keine Verbindungen mit dem Anwendungscode her (fügen Sie keine **Aktionen** oder **Outlets**hinzu).
- Fügen Sie `UIWebView` Objekte nicht hinzu.
...
4. Wählen Sie **IOS** > **Benutzeroberfläche** > **Startbildschirm** aus, und klicken Sie auf die Schaltfläche **weiter** :
...
10. Öffnen Sie im Abschnitt **App-Symbole und Start Bilder** die Dropdown Liste **Startbildschirm Datei** , und wählen Sie den oben erstellten `LaunchScreen.xib` aus:
...
13. Klicken Sie im **Projektmappen-Explorer**mit der rechten Maustaste auf den **Ressourcen** Ordner, und wählen Sie **Hinzufügen** > **Dateien hinzufügen...** aus:

    [![](unified-storyboards-images/dls09.png "Select Add / Add Files...")](unified-storyboards-images/dls09.png#lightbox)
14. Wählen Sie die oben erstellte `LaunchScreen.xib` Datei aus, und klicken Sie auf die Schaltfläche **Öffnen** :
...
Um die Abwärtskompatibilität mit IOS 7 aufrechtzuerhalten, nehmen Sie einfach die üblichen `Default.png` Image-Assets in der IOS 8-Anwendung auf. IOS kehrt zum vorherigen Verhalten zurück und verwendet diese Dateien als Startbildschirm, wenn es auf einem IOS 7-Gerät ausgeführt wird.
