...
- Datenbindung ist eine Technik zum Verknüpfen der Eigenschaften von zwei Objekten, damit Änderungen an einer Eigenschaft in der anderen Eigenschaft automatisch übernommen werden. Weitere Informationen finden Sie unter [Datenbindung](~/xamarin-forms/app-fundamentals/data-binding/index.md).
- Xamarin.Forms bietet es sich um eine Reihe von der anderen seitennavigationen, abhängig von den Seitentyp, der verwendet wird. Weitere Informationen finden Sie unter [Navigation](~/xamarin-forms/app-fundamentals/navigation/index.md).
- Stile können repetitivem Markup zu reduzieren, und ermöglichen eine Darstellung Anwendungen leichter geändert werden. Weitere Informationen finden Sie unter [Formatieren von Xamarin.Forms-Apps](~/xamarin-forms/user-interface/styles/index.md).
- XAML-Markuperweiterungen erweitern kann die Leistungsfähigkeit und Flexibilität von XAML, da Attribute des Elements aus anderen Quellen als literale Zeichenfolgen festgelegt werden. Weitere Informationen finden Sie unter [XAML-Markuperweiterungen](~/xamarin-forms/xaml/markup-extensions/index.md).
- Datenvorlagen bieten die Möglichkeit, die Darstellung von Daten auf unterstützten Ansichten definieren. Weitere Informationen finden Sie unter [Datenvorlagen](~/xamarin-forms/app-fundamentals/templates/data-templates/index.md).
- Jede Seite, die Layout und die Ansicht dargestellt wird anders jede Plattform mithilfe einer `Renderer` Klasse, die wiederum ein natives Steuerelement, erstellt die angeordnet, auf dem Bildschirm und fügt das in den freigegebenen Code angegebene Verhalten hinzu. Entwickler können ihre eigenen benutzerdefinierten `Renderer`-Klassen implementieren, um die Darstellung und/oder das Verhalten eines Steuerelements anzupassen. Weitere Informationen finden Sie unter [Custom Renderers](~/xamarin-forms/app-fundamentals/custom-renderer/index.md) (Benutzerdefinierte Renderer).
- Zudem können durch Effekte native Steuerelemente auf den einzelnen Plattformen angepasst werden. Effekte werden in plattformspezifischen Projekten erstellt, indem Unterklassen der [ `PlatformEffect` ](xref:Xamarin.Forms.PlatformEffect`2) Klasse, und werden durch das Anfügen an eine entsprechende Xamarin.Forms-Steuerelement. Weitere Informationen finden Sie unter [Effekte](~/xamarin-forms/app-fundamentals/effects/index.md).
