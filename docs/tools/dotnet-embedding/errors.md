...
Die Option `X`wird vom Tool nicht unterstützt. Möglicherweise wird dies von einer anderen Version des Tools unterstützt, `X` oder die Option wird in dieser Umgebung nicht angewendet.

<a name="EM0003" />

### <a name="em0003-the-platform-x-is-not-valid"></a>EM0003: die Plattform `X` ist ungültig.

Das Tool unterstützt die Plattform `X`nicht. Es ist möglich, dass dies von einer anderen Version des Tools unter `X` stützt wird oder die Plattform in dieser Umgebung nicht gilt.

<a name="EM0004" />

### <a name="em0004-the-target-x-is-not-valid"></a>EM0004: das Ziel `X` ist ungültig.

Das Tool unterstützt das Ziel `X`nicht. Es ist möglich, dass dies von einer anderen Version des Tools unter `X` stützt wird oder das Ziel in dieser Umgebung nicht gilt.

<a name="EM0005" />

### <a name="em0005-the-compilation-target-x-is-not-valid"></a>EM0005: das Kompilierungs `X` Ziel ist ungültig.

Das Tool unterstützt das Kompilierungs Ziel `X`nicht. Es ist möglich, dass dies von einer anderen Version des Tools unter `X` stützt wird oder das Kompilierungs Ziel in dieser Umgebung nicht gilt.
...
Die Syntax, die für die Befehlszeilen `A` Option angegeben wurde, konnte nicht vom Tool analysiert werden. Die richtige Syntax finden Sie in der Dokumentation.
...
Dies ist eine **Warnung** , dass der Konstruktor `C` ignoriert wird (d. h., es werden keine Elemente generiert), `T` da ein Parameter vom Typ nicht unterstützt wird.
...
Dies ist eine **Warnung** , dass die Standardparameter des Konstruktors `C` keinen zusätzlichen Code erzeugen. Die häufigste Ursache ist, dass eine vorhandene Methode bereits über dieselbe Signatur verfügt. In .net ist beispielsweise Folgendes möglich:
...
In solchen Fällen werden nur zwei `init` generierte Selektoren erstellt, die beide Mono aufrufen, aber es ist kein Wrapper für letztere vorhanden.

<a name="EM1030" />

### <a name="em1030-method-m-is-not-generated-because-return-type-t-is-not-supported"></a>EM1030: die `M` Methode wird nicht generiert, weil `T` der Rückgabetyp nicht unterstützt wird.
...
### <a name="em1032-method-m-has-default-values-for-which-no-wrapper-is-generated"></a>EM1032: die `M` Methode hat Standardwerte, für die kein Wrapper generiert wird.

Dies ist eine **Warnung** , dass die Standardparameter der `M` -Methode keinen zusätzlichen Code erzeugen. Die häufigste Ursache ist, dass eine vorhandene Methode bereits über dieselbe Signatur verfügt. In .net ist beispielsweise Folgendes möglich:
...
In solchen Fällen werden nur zwei `Increment` generierte Selektoren erstellt, die beide Mono aufrufen, aber es ist kein Wrapper für letztere vorhanden.
...
Dies ist eine **Warnung** , dass eine Erweiterungsmethode für einen priminungstyp (z. `System.Int32`b.) gefunden wurde. In Ziel-C ist es nicht möglich, Kategorien für einen primitiven Typ zu erstellen. Stattdessen erzeugt der Generator eine normale statische Methode.
...
### <a name="em1041-indexed-properties-on-t-is-not-generated-because-multiple-indexed-properties-are-not-supported"></a>EM1041: indizierte Eigenschaften `T` für werden nicht generiert, da mehrere indizierte Eigenschaften nicht unterstützt werden.
...
### <a name="em1051-element-e-is-generated-instead-as-f-because-its-name-conflicts-with-an-important-objective-c-selector"></a>EM1051: das `E` Element wird stattdessen als `F` generiert, da der Name mit einem wichtigen Ziel-c-Selektor in Konflikt steht.

Dies ist eine **Warnung** , dass das `E` -Element generiert `F` wird, da der Name mit einem wichtigen Ziel-c-Selektor in Konflikt steht.
...
### <a name="em1052-element-e-is-not-generated-its-name-conflicts-with-other-elements-on-the-same-class"></a>EM1052: das `E` Element wird nicht generiert. der Name steht in Konflikt mit anderen Elementen in derselben Klasse.
