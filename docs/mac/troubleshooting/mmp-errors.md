...
#### <a name="mm0001-this-version-of-xamarinmac-requires-mono-0-the-current-mono-version-is-1-please-update-the-monoframework-from-httpmono-projectcomdownloads"></a>MM0001: Diese Version von xamarin. Mac erfordert Mono {0} (die aktuelle Mono-Version ist {1}). Aktualisieren Sie die Mono. Framework- http://mono-project.com/Downloads

<a name="MM0003" />

#### <a name="mm0003-application-name-0exe-conflicts-with-an-sdk-or-product-assembly-dll-name"></a>MM0003: der Anwendungsname '{0}. exe ' verursacht einen Konflikt mit einem SDK-oder produktassemblynamen (. dll).
...
#### <a name="mm0008-you-should-provide-one-root-assembly-only-found-0-assemblies-1"></a>MM0008: Sie sollten nur eine Stammassembly bereitstellen, die {0} Assemblys gefunden wurde: "{1}"
...
#### <a name="mm0056-cannot-find-xcode-in-any-of-our-default-locations-please-install-xcode-or-pass-a-custom-path-using---sdkrootpath"></a>MM0056: Xcode kann in keinem unserer Standard Speicherorte gefunden werden. Installieren Sie Xcode, oder übergeben Sie einen benutzerdefinierten Pfad mit--SDKRoot =\<Pfad >
...
#### <a name="mm0071-unknown-platform--this-usually-indicates-a-bug-in-xamarinmac-please-file-a-bug-report-at-httpsbugzillaxamarincom-with-a-test-case"></a>MM0071: Unbekannte Plattform: *. Dies weist normalerweise auf einen Fehler in xamarin. Mac hin. Melden Sie einen Fehlerbericht auf https://bugzilla.xamarin.com mit einem Testfall.

Dies weist normalerweise auf einen Fehler in xamarin. Mac hin. Melden Sie einen Fehlerbericht auf [https://bugzilla.xamarin.com](https://bugzilla.xamarin.com/enter_bug.cgi?product=Xamarin.Mac) mit einem Testfall.

<a name="MM0073" />

#### <a name="mm0073-xamarinmac--does-not-support-a-deployment-target-of--the-minimum-is--please-select-a-newer-deployment-target-in-your-projects-infoplist"></a>MM0073: xamarin. Mac \* unterstützt kein Bereitstellungs Ziel \* (das minimal ist \*). Wählen Sie in der Datei "Info. plist" des Projekts ein neueres Bereitstellungs Ziel aus.
...
#### <a name="mm0074-xamarinmac--does-not-support-a-deployment-target-of--the-maximum-is--please-select-an-older-deployment-target-in-your-projects-infoplist-or-upgrade-to-a-newer-version-of-xamarinmac"></a>MM0074: xamarin. Mac \* unterstützt das Bereitstellungs Ziel \* nicht (das Maximum ist \*). Wählen Sie in der Info. plist-Datei des Projekts ein älteres Bereitstellungs Ziel aus, oder führen Sie ein Upgrade auf eine neuere Version von xamarin. Mac aus.
...
#### <a name="mm0097-machineconfig-file-0-can-not-be-found"></a>MM0097: die Machine. config-Datei "{0}" kann nicht gefunden werden.
...
Wenn `--runregistrar`ausgeführt wird, sollte mindestens eine Stammassembly bereitgestellt werden.
...
Wenn `--runregistrar`ausgeführt wird, sollte die Assemblyliste die produktassembly xamarin. Mac, xammac enthalten.
...
#### <a name="mm0133-found-more-than-1-assembly-matching-0-choosing-first-1"></a>MM0133: Es wurden mehr als 1 Assemblys gefunden, die mit "{0}" übereinstimmen, zuerst: "{1}
...
#### <a name="mm0135-did-not-link-system-framework-0-referenced-by-assembly-1-because-it-was-introduced-in-2-3-and-were-using-the-2-4-sdk"></a>MM0135: das System Framework '{0}' wurde nicht verknüpft (referenziert von Assembly '{1}'), weil es in {2} {3}eingeführt wurde, und wir verwenden das {2} {4} SDK.
...
#### <a name="mm1403-0-1-could-not-be-found-target-framework-0-is-unusable-to-package-the-application"></a>MM1403: {0} '{1}' wurde nicht gefunden. Das Ziel Framework '{0}' ist zum Packen der Anwendung nicht verwendbar.
...
#### <a name="mm1407-mismatch-between-xamarinmac-reference-0-and-target-framework-selected-1"></a>MM1407: Konflikt zwischen xamarin. Mac-Verweis "{0}" und Ziel Framework "{1}" ausgewählt.
...
#### <a name="mm1603-unknown-format-for-fat-entry-at-position-0-in-1"></a>MM1603: unbekanntes Format für FAT-Eintrag an der Position {0} in {1}.
...
#### <a name="mm2004-extra-linker-definitions-file-0-could-not-be-located"></a>MM2004: die zusätzliche linkerdefinitions-Datei "{0}" konnte nicht gefunden werden.

<a name="MM2005" />

#### <a name="mm2005-definitions-from-0-could-not-be-parsed"></a>MM2005: Definitionen aus "{0}" konnten nicht analysiert werden.

<a name="MM2006" />

#### <a name="mm2006-native-library-0-was-referenced-but-could-not-be-found"></a>MM2006: auf die native Bibliothek '{0}' wurde verwiesen, Sie wurde jedoch nicht gefunden.
...
#### <a name="mm2012-only-first-0-of-1-referenced-by-warnings-shown--this-message-related-to-2009-"></a>MM2012: Es werden nur die ersten {0} von {1} Warnungen angezeigt, auf die verwiesen wird. \*\* diese Meldung im Zusammenhang mit 2009 \*\*

<a name="MM2013" />

#### <a name="mm2013-failed-to-resolve-the-reference-to-0-referenced-in-1-the-app-will-not-include-the-referenced-assembly-and-may-fail-at-runtime"></a>MM2013: Fehler beim Auflösen des Verweises auf "{0}", auf den in "{1}" verwiesen wird. Die app enthält nicht die Assembly, auf die verwiesen wird, und kann zur Laufzeit fehlschlagen.
...
#### <a name="mm2016-invalid-tlsprovider-0-option-the-only-valid-value-1-will-be-used"></a>MM2016: Ungültige Option "tlsprovider `{0}`". Der einzige gültige Wert `{1}` der verwendet wird.
...
#### <a name="mm202x-binding-optimizer-failed-processing-"></a>MM202x: Fehler beim Verarbeiten `...`durch den Bindungs Optimierer.
...
#### <a name="mm2106-could-not-optimize-the-call-to-blockliteralsetupblockunsafe-in--at-offset--because-"></a>MM2106: der aufrufsblock "blockliterale. setupblock [unsicher]" konnte in \* bei Offset \* nicht optimiert werden, da \*.

Der Linker meldet diese Warnung, wenn er einen aufzurufenden `BlockLiteral.SetupBlock` oder `Block.SetupBlockUnsafe`nicht optimieren kann.

Die Meldung verweist auf die Methode, die `BlockLiteral.SetupBlock[Unsafe]`aufruft, und kann auch darauf hinweisen, warum der Aufruf nicht optimiert werden konnte.
...
Der Linker meldet diese Warnung, wenn der Entwickler das Entfernen der dynamischen Registrierungsstelle anfordert (indem er `--optimize:remove-dynamic-registrar` an MMP übergibt), aber der Linker ermittelt, dass dies nicht sicher ist.

Um die Warnung zu entfernen, entfernen Sie entweder das Optimierungs Argument zu MMP, oder übergeben Sie `--nowarn:2107`, um es zu ignorieren.

Diese Option wird standardmäßig automatisch aktiviert, wenn dies möglich und sicher ist.

<a name="MM2108" />

#### <a name="mm2108-0-was-stripped-of-architectures-except-1-to-comply-with-app-store-restrictions-this-could-break-exisiting-codesigning-signatures-consider-stripping-the-library-with-lipo-or-disabling-with---optimize-trim-architectures"></a>MM2108: '{0}' wurde von Architekturen außer '{1}' entfernt, um App Store-Einschränkungen einzuhalten. Dies könnte das Beenden der Signatur des Codierung unterbrechen. Entfernen Sie die Bibliothek mit der Lipo, oder deaktivieren Sie mit--optimieren =-Trim-Architekturen);
...
Verwenden Sie `lipo`, um die unnötigen Architekturen dauerhaft aus der Quell Bibliothek zu entfernen. Wenn die Anwendung nicht im App Store veröffentlicht wird, kann diese Entfernung deaktiviert werden, indem `--optimize=-trim-architectures` als zusätzliche MMP-Argumente übergeben wird.
...
#### <a name="mm3010-exclusion-of-aot-of-0-was-requested-but-was-not-found"></a>MM3010: der Ausschluss von "{0}" wurde angefordert, wurde jedoch nicht gefunden.
...
#### <a name="mm4134-your-application-is-using-the-0-framework-which-isnt-included-in-the-macos-sdk-youre-using-to-build-your-app-this-framework-was-introduced-in-osx-2-while-youre-building-with-the-macos-1-sdk-this-configuration-is-not-supported-with-the-static-registrar-pass---registrardynamic-as-an-additional-mmp-argument-in-your-projects-mac-build-option-to-select-alternatively-select-a-newer-sdk-in-your-apps-mac-build-options"></a>MM4134: Ihre Anwendung verwendet das Framework "{0}", das nicht im MacOS SDK enthalten ist, das Sie zum Erstellen Ihrer APP verwenden (dieses Framework wurde in OSX {2}eingeführt, während Sie mit dem MacOS {1} SDK erstellen.) Diese Konfiguration wird bei der statischen Registrierungsstelle (Pass--Registrar: Dynamic als zusätzliches MMP-Argument in der Mac-Buildoption Ihres Projekts für die Auswahl) nicht unterstützt. Wählen Sie alternativ ein neueres SDK in den Mac-Buildoptionen Ihrer APP aus.
...
#### <a name="mm5101-missing-0-compiler-please-install-xcode-command-line-tools-component"></a>MM5101: fehlender "{0}"-Compiler. Installieren Sie die Xcode-Komponente "Befehlszeilen Tools".

<!-- 5102 used by mtouch -->

<a name="MM5103" />

#### <a name="mm5103-failed-to-compile-error-code---0-please-file-a-bug-report-at-httpbugzillaxamarincom"></a>MM5103: Kompilierung fehlgeschlagen. Fehlercode-{0}. Melden Sie einen Fehlerbericht unter http://bugzilla.xamarin.com
...
#### <a name="mm5301-pkg-config-could-not-be-found-please-install-the-monoframework-from-httpmono-projectcomdownloads"></a>MM5301: pkg-config konnte nicht gefunden werden. Installieren Sie Mono. Framework aus http://mono-project.com/Downloads
...
Dies weist auf einen Fehler in xamarin. Mac hin. Bitte melden Sie einen Fehler auf [https://bugzilla.xamarin.com](https://bugzilla.xamarin.com/enter_bug.cgi?product=Xamarin.Mac).

Eine mögliche Problem Umgehung besteht darin, die `register-protocols` Optimierung zu deaktivieren, indem Sie `--optimize:-register-protocols` als zusätzliches MMP-Argument in den Mac-Buildoptionen des Projekts übergeben.
...
Dies weist normalerweise auf einen Fehler in xamarin. Mac hin, da die dynamische Registrierungsstelle bei Bedarf nicht verknüpft werden sollte. Bitte melden Sie einen Fehler auf [https://bugzilla.xamarin.com](https://bugzilla.xamarin.com/enter_bug.cgi?product=iOS).
