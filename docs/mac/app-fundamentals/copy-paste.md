...
3. Geben Sie `MacCopyPaste` als **Projektnamen** ein, und behalten Sie den Standardwert bei. Klicken Sie auf „Weiter: 
...
Der folgende Code stellt eine-Eigenschaft bereit, um zu testen, ob Bilddaten auf dem Standard-pasteboard vorhanden sind. Wenn ein Bild verfügbar ist, `true` wird else zurückgegeben `false` :
...
Mithilfe von `Export` , `WillChangeValue` und `DidChangeValue` haben wir die- `Document` Eigenschaft so eingerichtet, dass die Schlüssel-Wert-Codierung und Datenbindung in Xcode zulässig ist.
...
Bearbeiten Sie die Datei **AppDelegate.cs** , und führen Sie die `DidFinishLaunching` folgende Methode aus:
...
In den meisten Fällen verwenden Sie eines der vom System definierten pasteboards. Es kann jedoch Situationen geben, in denen Sie Ihre eigenen pasteboards erstellen müssen. In diesen Fällen können Sie die- `FromName (string name)` Methode der-Klasse verwenden, `NSPasteboard` um ein benutzerdefiniertes "Zwischenablage" mit dem angegebenen Namen zu erstellen.

Optional können Sie die- `CreateWithUniqueName` Methode der-Klasse aufzurufen `NSPasteboard` , um ein eindeutig benanntes "pasteboard" zu erstellen.
...
Jede Darstellung wird über einen URI (Uniform Type Identifier) identifiziert. Dies ist nichts anderes als eine einfache Zeichenfolge, die den Typ des dargestellten Datums eindeutig identifiziert (Weitere Informationen finden Sie in der [Übersicht über die Uniform Type Identifiers](https://developer.apple.com/library/prerelease/mac/documentation/FileManagement/Conceptual/understanding_utis/understand_utis_intro/understand_utis_intro.html#//apple_ref/doc/uid/TP40001319) -Dokumentation von Apple). 
...
Verwenden Sie die- `ChangeCount` Methode und die `ClearContents` -Methode der `NSPasteboard` -Klasse, um die Änderungs Anzahl eines bestimmten pasteboards zu ändern.
...
In der Regel schreiben Sie einfach auf das allgemeine "pasteboard", wie im obigen Beispiel gezeigt. Alle Objekte, die Sie an die-Methode senden, `WriteObjects` *müssen* mit der- `INSPasteboardWriting` Schnittstelle übereinstimmen. Mehrere integrierte Klassen (z `NSString` `NSImage` . b.,,, `NSURL` `NSColor` , `NSAttributedString` und `NSPasteboardItem` ) entsprechen automatisch dieser Schnittstelle.

Wenn Sie eine benutzerdefinierte Datenklasse in das Zwischenablage schreiben, muss Sie der- `INSPasteboardWriting` Schnittstelle entsprechen oder in eine Instanz der-Klasse umgeschrieben werden `NSPasteboardItem` (siehe Abschnitt " [benutzerdefinierte Datentypen](#Custom_Data_Types) " unten).
...
Mithilfe der-Methode lesen Sie Daten aus dem-Steuer Board `ReadObjectsForClasses` . Es sind zwei Parameter erforderlich:
...
1. Erstellen Sie einen einzelnen `ReadObjectsForClasses` -Befehl an die-Methode, und stellen Sie ein Array aller Darstellungen bereit, die Sie wünschen (in der bevorzugten Reihenfolge).
2. Nehmen Sie mehrere Aufrufe an die-Methode an, die `ReadObjectsForClasses` jedes Mal ein anderes Array von Typen anfordern.
...
In dieser Situation müssen Sie die benutzerdefinierte Datenklasse so entwerfen, dass Sie von erbt `NSObject` und einigen wenigen Schnittstellen entspricht ( `INSCoding` `INSPasteboardWriting` und `INSPasteboardReading` ). Optional können Sie einen verwenden, `NSPasteboardItem` um die zu kopierenden oder einzufügenden Daten zu kapseln.
...
Bevor eine benutzerdefinierte Datenklasse in ein "pasteboard" geschrieben oder daraus gelesen werden kann, muss Sie den `INSPastebaordWriting` -und- `INSPasteboardReading` Schnittstellen entsprechen. Außerdem muss Sie von Erben `NSObject` und auch der- `INSCoding` Schnittstelle entsprechen:
...
Die-Klasse muss auch mit der-Direktive für "Ziel-C" verfügbar gemacht werden, `Register` und Sie muss alle erforderlichen Eigenschaften oder Methoden mit verfügbar machen `Export` . Beispiel:
...
Zuerst machen wir den _leeren_ Konstruktor unter der Standardmethode "Ziel-C" von verfügbar `init` .

Als nächstes machen wir einen `NSCoding` kompatiblen Konstruktor verfügbar, der verwendet wird, um eine neue Instanz des Objekts aus dem Zwischenablage zu erstellen, wenn Sie unter den exportierten Namen von einfügen `initWithCoder` .

Dieser Konstruktor nimmt einen `NSCoder` (wie von einem erstellt `NSKeyedArchiver` , wenn er in das-Steuerelement geschrieben wird), extrahiert die gekoppelten Schlüssel-Wert-Daten und speichert Sie in den Eigenschaften Feldern der Datenklasse.
...
Für unser benutzerdefiniertes Format erstellen wir unseren eigenen Bezeichner: "com. xamarin. Image-Info" (Beachten Sie, dass sich in umgekehrter Schreibweise wie ein App-Bezeichner befindet). Unsere Klasse ist auch in der Lage, eine Standard Zeichenfolge in das Zwischenablage () zu schreiben `public.text` . 
...
Derzeit ist nur die `WritingPromised` -Option verfügbar und sollte verwendet werden, wenn ein bestimmter Typ nur eine Zusage erhält, die eigentlich nicht in das pasteboard geschrieben wurde. Weitere Informationen finden Sie weiter oben im Abschnitt " [versprochene Daten](#Promised_Data) ".
...
Für den- `com.xamarin.image-info` Typ weisen wir das Zwischenablage an, das Schlüssel-Wert-Paar, das wir mit erstellt haben, zu decodieren, `NSKeyedArchiver` Wenn die Klasse durch Aufrufen des `initWithCoder:` Konstruktors, den wir der Klasse hinzugefügt haben, in das Zwischenablage geschrieben wurde.
...
Um benutzerdefinierte Daten in einen zu schreiben, `NSPasteboardItem` müssen Sie ein benutzerdefiniertes bereitstellen `NSPasteboardItemDataProvider` . Fügen Sie dem Projekt eine neue Klasse hinzu, und nennen Sie Sie **ImageInfoDataProvider.cs**. Bearbeiten Sie die Datei, und führen Sie Sie wie folgt aus:
...
Wie bei der benutzerdefinierten Datenklasse, müssen wir die `Register` -Direktive und die- `Export` Direktive verwenden, um Sie für "Ziel-C" verfügbar zu machen. Die Klasse muss von Erben `NSPasteboardItemDataProvider` und die `FinishedWithDataProvider` -Methode und die- `ProvideDataForType` Methode implementieren.

Verwenden Sie die- `ProvideDataForType` Methode, um die Daten bereitzustellen, die in den wie folgt umschließt werden `NSPasteboardItem` :
