...
Ereignisse in .NET implementieren das Veröffentlichen-Abonnieren-Muster und sind der einfachste und unkomplizierte Ansatz für eine Kommunikationsschicht zwischen Komponenten, wenn keine lose Kopplung erforderlich ist, wie z. b. ein Steuerelement und die Seite, die es enthält. Die Lebensdauer von Verlegern und Abonnenten wird jedoch durch Objekt Verweise miteinander gekoppelt, und der Abonnenten Typ muss einen Verweis auf den Verlegertyp aufweisen. Dadurch können Probleme bei der Speicherverwaltung entstehen, insbesondere dann, wenn kurzlebige Objekte vorhanden sind, die ein Ereignis eines statischen oder langlebigen Objekts abonnieren. Wenn der Ereignishandler nicht entfernt wird, wird der Abonnent durch den Verweis auf ihn auf dem Verleger beibehalten. Dadurch wird die Garbage Collection des Abonnenten verhindert oder verzögert.
...
![](communicating-between-loosely-coupled-components-images/messagingcenter.png "Multicast-Funktionen zum Veröffentlichen und Abonnieren")
...
Der eshoponcontainers-Mobile App verwendet [`MessagingCenter`](xref:Xamarin.Forms.MessagingCenter) die-Klasse, um zwischen lose gekoppelten Komponenten zu kommunizieren. Die APP definiert drei Nachrichten:

- Die `AddProduct` Meldung wird von der `CatalogViewModel` -Klasse veröffentlicht, wenn dem Warenkorb ein Element hinzugefügt wird. In der Rückgabe abonniert die `BasketViewModel` -Klasse die Nachricht und erhöht die Anzahl der Elemente im Warenkorb als Antwort. Außerdem wird von der `BasketViewModel` -Klasse auch die abonnierten dieser Nachricht abonniert.
- Die `Filter` Meldung wird von der `CatalogViewModel` -Klasse veröffentlicht, wenn der Benutzer einen Marken-oder Typfilter auf die Elemente anwendet, die aus dem Katalog angezeigt werden. In der Rückgabe abonniert die `CatalogView` -Klasse die Nachricht und aktualisiert die Benutzeroberfläche, sodass nur Elemente angezeigt werden, die den Filterkriterien entsprechen.
- Die `ChangeTab` Meldung wird von der `MainViewModel` -Klasse veröffentlicht, `CheckoutViewModel` wenn der zu der `MainViewModel` folgenden erfolgreichen Erstellung und Übermittlung einer neuen Bestellung navigiert. In der Rückgabe abonniert die `MainView` -Klasse die Nachricht und aktualisiert die Benutzeroberfläche, sodass die Registerkarte **mein Profil** aktiv ist, um die Bestellungen des Benutzers anzuzeigen.
...
Herausgeber benachrichtigen Abonnenten einer Nachricht mit einer der [`MessagingCenter.Send`](xref:Xamarin.Forms.MessagingCenter.Send*)-Überladungen. Im folgenden Codebeispiel wird das Veröffentlichen `AddProduct` der Nachricht veranschaulicht:
...
Abonnenten können sich registrieren, um eine Nachricht mit einer der [`MessagingCenter.Subscribe`](xref:Xamarin.Forms.MessagingCenter.Subscribe*)-Überladungen zu empfangen. Im folgenden Codebeispiel wird veranschaulicht, wie der eshoponcontainers-Mobile App die `AddProduct` Nachricht abonniert und verarbeitet:
...
In diesem Beispiel abonniert [`Subscribe`](xref:Xamarin.Forms.MessagingCenter.Subscribe*) die-Methode die `AddProduct` Meldung und führt als Antwort auf den Empfang der Nachricht einen Rückruf Delegaten aus. Dieser Rückruf Delegat, der als Lambda-Ausdruck angegeben wird, führt Code aus, der die Benutzeroberfläche aktualisiert.
...
Ein Abonnent muss möglicherweise nicht jede Instanz einer veröffentlichten Nachricht verarbeiten, und dies kann durch die generischen Typargumente gesteuert werden, die für die [`Subscribe`](xref:Xamarin.Forms.MessagingCenter.Subscribe*)-Methode angegeben werden. In diesem Beispiel empfängt `AddProduct` der Abonnent nur Nachrichten, die von der `CatalogViewModel` -Klasse gesendet werden, deren Nutzlastdaten `CatalogItem` eine-Instanz ist.
