...
In diesem Artikel wird erläutert, wie Sprache-zu-Text in der Beispiel Xamarin.Forms Anwendung mithilfe des Azure Speech Service implementiert wird. Die folgenden Screenshots zeigen die Beispielanwendung unter IOS und Android:
...
Jede Plattform erfordert die Berechtigung für den Zugriff auf das Mikrofon. Das Beispiel Projekt stellt eine `IMicrophoneService` Schnittstelle im freigegebenen Projekt bereit und verwendet zum Xamarin.Forms `DependencyService` Abrufen von Platt Form Implementierungen der-Schnittstelle.
...
Das Beispiel Projekt definiert ein einfaches Seitenlayout in der Datei " **MainPage. XAML** ". Bei den schlüssellayoutelementen handelt es sich um einen, der `Button` den Aufzeichnungsprozess startet, eine, `Label` die den Text mit Text enthält, und einen, der angezeigt wird, wenn die Aufzeichnung ausgeführt `ActivityIndicator` wird
...
Der- `MainPage` Konstruktor ruft eine Instanz der- `IMicrophoneService` Schnittstelle aus ab `DependencyService` :
...
1. Erstellt eine Instanz der- `SpeechRecognizer` Klasse, wenn Sie nicht bereits vorhanden ist.
...
Die übrigen `MainPage` Klassen Methoden sind Hilfsprogramme zum Anzeigen des Anwendungs Zustands:
...
Die- `UpdateTranscription` Methode schreibt den bereitgestellten `newText` `string` in das-Element mit dem `Label` Namen `transcribedText` . Es erzwingt, dass dieses Update im UI-Thread ausgeführt wird, sodass es von jedem Kontext aus aufgerufen werden kann, ohne dass Ausnahmen ausgelöst werden. `InsertDateTimeRecord`Schreibt das aktuelle Datum und die aktuelle Uhrzeit in die- `transcribedText` Instanz, um den Anfang einer neuen Aufzeichnung zu markieren. Schließlich aktualisiert die `UpdateDisplayState` -Methode das `Button` -Element und das- `ActivityIndicator` Element, um widerzuspiegeln, ob eine Aufzeichnung ausgeführt wird.
...
Das Beispiel Projekt definiert eine `IMicrophoneService` Implementierung für Android mit dem Namen `AndroidMicrophoneService` :
...
`AndroidMicrophoneService`Verfügt über die folgenden Features:

1. Das- `Dependency` Attribut registriert die-Klasse beim `DependencyService` .
1. Die `GetPermissionAsync` -Methode überprüft, ob auf der Grundlage der Android SDK Version Berechtigungen erforderlich sind, und ruft auf, `RequestMicPermissions` Wenn die Berechtigung nicht bereits erteilt wurde.
1. Die `RequestMicPermissions` -Methode verwendet die `Snackbar` -Klasse, um Berechtigungen vom Benutzer anzufordern, wenn eine Begründung erforderlich ist; andernfalls werden direkt audioaufzeichnungs Berechtigungen angefordert.
...
Die- `MainActivity` Klasse wird angepasst, um die Instanz zu aktualisieren, `AndroidMicrophoneService` Wenn Berechtigungsanforderungen erfüllt sind:
...
Das Beispiel Projekt definiert eine `IMicrophoneService` Implementierung für IOS mit dem Namen `iOSMicrophoneService` :
...
`iOSMicrophoneService`Verfügt über die folgenden Features:

1. Das- `Dependency` Attribut registriert die-Klasse beim `DependencyService` .
1. Die- `GetPermissionAsync` Methode ruft `RequestMicPermissions` auf, um Berechtigungen vom Gerätebenutzer anzufordern.
1. Die- `RequestMicPermissions` Methode verwendet die freigegebene- `AVAudioSession` Instanz, um Aufzeichnungs Berechtigungen anzufordern.
...
Das Beispiel Projekt definiert eine `IMicrophoneService` Implementierung für UWP mit dem Namen `UWPMicrophoneService` :
...
`UWPMicrophoneService`Verfügt über die folgenden Features:

1. Das- `Dependency` Attribut registriert die-Klasse beim `DependencyService` .
1. Die- `GetPermissionAsync` Methode versucht, eine-Instanz zu initialisieren `MediaCapture` . Wenn dies nicht der Fall ist, wird eine Benutzer Anforderung zum Aktivieren des Mikrofons gestartet.
...
- [Schnellstart: Erkennen von Spracheingaben per Mikrofon](https://docs.microsoft.com/azure/cognitive-services/speech-service/quickstarts/speech-to-text-from-microphone)