...
Wenn die `BindingContext`-Eigenschaft von `DatePicker` auf eine ViewModel-Instanz festgelegt ist, die Eigenschaften des Typs `DateTime` namens `MinDate`, `MaxDate`und `SelectedDate` enthält (z. b.), können Sie die `DatePicker` wie folgt instanziieren:
...
Wenn das Beispiel zuerst ausgeführt wird, sowohl `DatePicker` Ansichten werden initialisiert, das heutige Datum. Der folgende Screenshot zeigt das Programm, das unter IOS und Android ausgeführt wird:
