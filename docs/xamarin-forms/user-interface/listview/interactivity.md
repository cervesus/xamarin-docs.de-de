...
Häufig möchten Benutzeraktionen für ein Element in einer `ListView`durchführen. Betrachten Sie beispielsweise eine Liste von e-Mails in der Mail-app. Unter IOS können Sie eine Nachricht löschen, um eine Nachricht zu löschen:
...
Benutzer schon lange davon ausgehen, dass in der Liste der Daten nach unten ziehen, Liste aktualisiert. Das [`ListView`](xref:Xamarin.Forms.ListView) -Steuerelement unterstützt diese standardmäßig. Legen Sie zum Aktivieren der Pull-to-refresh-Funktionalität [`IsPullToRefreshEnabled`](xref:Xamarin.Forms.ListView.IsPullToRefreshEnabled) auf `true`fest:
