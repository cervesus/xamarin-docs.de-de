...
---:| | `Clear`    |             |              |        | | `Src`      |             | Quelle | X | | `Dst`      | X | Ziel |        | | `SrcOver`  | X | Quelle | X | | `DstOver`  | X | Ziel | X | | `SrcIn`    |             | Quelle |        | | `DstIn`    |             | Ziel |        | | `SrcOut`   |             |              | X | | `DstOut`   | X |              |        | | `SrcATop`  | X | Quelle |        | | `DstATop`  |             | Ziel | X | | `Xor`      | X |              | X | | `Plus`     | X | Summe | X | | `Modulate` |             | Produkt |        | 
