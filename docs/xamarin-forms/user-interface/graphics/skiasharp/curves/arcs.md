...
Das- `startAngle` Argument ist ein Winkel im Uhrzeigersinn in Grad, der relativ zu einer horizontalen Linie ist, die von der Mitte der Ellipse nach rechts gezeichnet wird. Das- `sweepAngle` Argument ist relativ zum `startAngle` . Dies sind `startAngle` die `sweepAngle` Werte von 60 Grad bzw. 100 Grad:
...
Die Kurve, die dem Pfad mit der `AddArc` - `ArcTo` Methode oder der-Methode hinzugefügt wird, ist einfach der Teil des Umfangs der Ellipse:

![Der Winkel Bogen allein](arcs-images/anglearc.png)

Das `startAngle` -Argument oder das- `sweepAngle` Argument kann negativ sein: der Bogen ist im Uhrzeigersinn für positive Werte von `sweepAngle` und gegen den Uhrzeigersinn bei negativen Werten.

Definiert jedoch `AddArc` *keine* geschlossene Kontur. Wenn Sie `LineTo` after aufgerufen `AddArc` haben, wird eine Linie vom Ende des Bogens bis zum Punkt in der `LineTo` -Methode gezeichnet, und dasselbe gilt für `ArcTo` .

`AddArc`startet automatisch eine neue Kontur und ist funktional äquivalent zu einem-Befehl `ArcTo` mit einem abschließenden Argument von `true` :

```csharp
path.ArcTo (oval, startAngle, sweepAngle, true);
```

Das letzte Argument wird aufgerufen `forceMoveTo` und bewirkt, dass es einen `MoveTo` Aufruf am Anfang des Bogens auslöst. Dadurch wird eine neue Kontur begonnen. Dies ist nicht der Fall mit dem letzten Argument von `false` :
...
Auf der Seite " **Winkel Bogen** " können Sie zwei Schieberegler verwenden, um die Start-und Sweep-Winkel anzugeben. In der XAML-Datei werden zwei `Slider` -Elemente und ein-Element instanziiert `SKCanvasView` . Der `PaintCanvas` -Handler in der [**AngleArcPage.XAML.cs**](https://github.com/xamarin/xamarin-forms-samples/blob/master/SkiaSharpForms/Demos/Demos/SkiaSharpFormsDemos/Curves/AngleArcPage.xaml.cs) -Datei zeichnet sowohl das Oval als auch den Bogen mit zwei Objekten, die `SKPaint` als Felder definiert sind:
...
Ein neues- `SKPath` Objekt wird für jeden Kreis Slice erstellt. Der Pfad besteht aus einer Zeile aus der Mitte, einem `ArcTo` zum Zeichnen des Bogens und einer anderen Zeile zurück zum Mittelpunkt des `Close` Aufrufes. Dieses Programm zeigt "explodierte" Kreis Scheiben an, indem Sie alle von der Mitte um 50 Pixel verschoben werden. Diese Aufgabe erfordert einen Vektor in der Richtung des Mittelpunkts des Sweep-Winkels für jeden Slice:
...
Um zu sehen, wie es ohne die "Explosion" aussieht, kommentieren Sie einfach den-Befehl aus `Translate` :
...
Der zweite von unterstützte Arkus `SKPath` Bogen ist der *Tangens Bogen*, der aufgerufen wird, da der Bogen der Umfang eines Kreises ist, der sich auf zwei verbundene Linien Tangens hat.

Einem Pfad wird ein Tangens Bogen mit einem Aufrufen der- [`ArcTo`](xref:SkiaSharp.SKPath.ArcTo(SkiaSharp.SKPoint,SkiaSharp.SKPoint,System.Single)) Methode mit zwei `SKPoint` Parametern oder der Überladung [`ArcTo`](xref:SkiaSharp.SKPath.ArcTo(System.Single,System.Single,System.Single,System.Single,System.Single)) mit separaten `Single` Parametern für die Punkte hinzugefügt:
...
Diese `ArcTo` Methode ähnelt der [`arct`](https://www.adobe.com/products/postscript/pdfs/PLRM.pdf) Funktion PostScript (Page 532) und der IOS- [`AddArcToPoint`](xref:CoreGraphics.CGPath.AddArcToPoint(System.nfloat,System.nfloat,System.nfloat,System.nfloat,System.nfloat)) Methode.

Die `ArcTo` Methode umfasst drei Punkte:

- Der aktuelle Punkt der Kontur oder der Punkt (0,0), wenn `MoveTo` nicht aufgerufen wurde.
- Das erste Punkt Argument der `ArcTo` Methode, die als *Eckpunkt* bezeichnet wird.
- Das zweite Punkt Argument für `ArcTo` , das als *Zielpunkt*bezeichnet wird:
...
Wenn die drei Punkte colinear sind, &mdash; d. h., wenn Sie sich in derselben geraden Geraden befinden, &mdash; wird kein Bogen gezeichnet.
...
Die Kurve, die der Kontur hinzugefügt wird, berührt keines der Punkte, die in der-Methode angegeben sind `ArcTo` . Er besteht aus einer geraden Linie zwischen dem aktuellen Punkt und dem ersten Tangens Punkt und einem Bogen, der am zweiten Tangenten Punkt endet, der hier rot dargestellt wird:
...
Die Seite **Tangens Bogen** ermöglicht Ihnen das Experimentieren mit dem Tangens Bogen. Dies ist die erste von mehreren Seiten, die von abgeleitet [`InteractivePage`](https://github.com/xamarin/xamarin-forms-samples/blob/master/SkiaSharpForms/Demos/Demos/SkiaSharpFormsDemos/InteractivePage.cs) werden, die einige praktische `SKPaint` Objekte definiert und `TouchPoint` Verarbeitungsvorgänge ausführt:
...
Der `PaintSurface` Handler verwendet die `ArcTo` -Methode, um den Bogen basierend auf den Berührungspunkten und einem zu zeichnen `Slider` , aber berechnet auch algorithmisch den Kreis, auf dem der Winkel basiert:
...
Der elliptische Bogen wird einem Pfad hinzugefügt, indem die-Methode aufgerufen wird, die [`ArcTo`](xref:SkiaSharp.SKPath.ArcTo(SkiaSharp.SKPoint,System.Single,SkiaSharp.SKPathArcSize,SkiaSharp.SKPathDirection,SkiaSharp.SKPoint)) über zwei `SKPoint` Parameter verfügt, oder die-Überladung [`ArcTo`](xref:SkiaSharp.SKPath.ArcTo(System.Single,System.Single,System.Single,SkiaSharp.SKPathArcSize,SkiaSharp.SKPathDirection,System.Single,System.Single)) mit separaten X-und Y-Koordinaten:
...
Der elliptische Bogen ist mit dem [Ellipsen Bogen](https://www.w3.org/TR/SVG11/paths.html#PathDataEllipticalArcCommands) konsistent, der in der universelle Windows-Plattform-Klasse enthalten ist [`ArcSegment`](/uwp/api/Windows.UI.Xaml.Media.ArcSegment/) .
...
Wenn die gekippte Ellipse nicht groß genug ist, um zwischen den beiden Punkten zu passen, wird Sie einheitlich skaliert, bis Sie groß genug ist. Nur zwei eindeutige Bögen verbinden die beiden Punkte in diesem Fall. Diese können mit dem-Parameter unterschieden werden `SKPathDirection` .
...
Mithilfe der **Ellipsen Bogen** Seite können Sie die beiden Punkte sowie die Größe und Drehung der Ellipse interaktiv festlegen. Die `EllipticalArcPage` -Klasse wird von abgeleitet `InteractivePage` , und der- `PaintSurface` Handler in der [**EllipticalArcPage.XAML.cs**](https://github.com/xamarin/xamarin-forms-samples/blob/master/SkiaSharpForms/Demos/Demos/SkiaSharpFormsDemos/Curves/EllipticalArcPage.xaml.cs) -Code Behind-Datei zeichnet die vier Arcs:
...
Der `PaintSurface` Handler in der- [`ArcInfinityPage`](https://github.com/xamarin/xamarin-forms-samples/blob/master/SkiaSharpForms/Demos/Demos/SkiaSharpFormsDemos/Curves/ArcInfinityPage.cs) Klasse positioniert das Unendlichkeitszeichen so, dass sich der (0,0) Punkt in der Mitte der Seite befindet, und skaliert den Pfad auf die Bildschirmgröße:
...
Im Code wird die- `Bounds` Eigenschaft von verwendet `SKPath` , um die Dimensionen des unendlichen Sinus zu bestimmen, um Sie auf die Größe der Canvas zu skalieren:
...
Das Ergebnis scheint etwas kleiner zu sein. Dies deutet darauf hin, dass die- `Bounds` Eigenschaft von `SKPath` eine Größe meldet, die größer ist als der Pfad.
...
Um eine strengere Anpassung zu erzielen, verwenden Sie die- `TightBounds` Eigenschaft, die die Steuerpunkte ausschließt. Hier ist das Programm, das im Querformat ausgeführt wird, und die Verwendung der- `TightBounds` Eigenschaft zum Abrufen der Pfad Begrenzungen:
