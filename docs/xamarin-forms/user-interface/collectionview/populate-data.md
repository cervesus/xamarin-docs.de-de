...
> [!WARNING]
> [`CollectionView`](xref:Xamarin.Forms.CollectionView)löst eine Ausnahme aus, wenn [`ItemsSource`](xref:Xamarin.Forms.ItemsView.ItemsSource) die vom UI-Thread aktualisiert wird.
...
Weitere Informationen zum Ändern des [`CollectionView`](xref:Xamarin.Forms.CollectionView) Layouts finden Sie unter [xamarin. Forms CollectionView-Layout](layout.md). Informationen dazu, wie die Darstellung der einzelnen Elemente in der `CollectionView`definiert wird, finden Sie unter Definieren der [Element](#define-item-appearance)Darstellung.
...
[`CollectionView`](xref:Xamarin.Forms.CollectionView)unterstützt Kontextmenüs für Datenelemente über das `SwipeView`-Element, das das Kontextmenü mit einer Schwenkbewegung zeigt. Der `SwipeView` ist ein Container Steuerelement, das ein Element des Inhalts umschließt und Kontextmenü Elemente für das Inhalts Element bereitstellt. Daher werden Kontextmenüs für ein `CollectionView` implementiert, indem ein `SwipeView` erstellt wird, das den Inhalt definiert `SwipeView` , den der umschließt, sowie die Kontextmenü Elemente, die durch die Schwenkbewegung angezeigt werden. Dies wird erreicht, indem `SwipeView` als Stamm Ansicht in der [`DataTemplate`](xref:Xamarin.Forms.DataTemplate) festgelegt wird, die die Darstellung der einzelnen Datenelemente in der `CollectionView`definiert:
...
In diesem Beispiel ist der `SwipeView` Inhalt eine [`Grid`](xref:Xamarin.Forms.Grid) , die die Darstellung der einzelnen Elemente in der [`CollectionView`](xref:Xamarin.Forms.CollectionView)definiert. Die wischen-Elemente werden verwendet, um Aktionen für `SwipeView` den Inhalt auszuführen. Sie werden angezeigt, wenn das Steuerelement von der linken Seite entfernt wird:
...
[`CollectionView`](xref:Xamarin.Forms.CollectionView)unterstützt die Funktion zum Aktualisieren von `RefreshView`Pull-Funktionen über das-Element, das die Aktualisierung der angezeigten Daten durch Abrufen der Liste der Elemente ermöglicht. Bei `RefreshView` handelt es sich um ein Container Steuerelement, das dem untergeordneten Pullvorgang die Aktualisierungs Funktionalität bereitstellt Der Pull-Vorgang zum Aktualisieren wird daher für `CollectionView` ein implementiert, indem er als untergeordnetes `RefreshView`Element eines festgelegt wird:
