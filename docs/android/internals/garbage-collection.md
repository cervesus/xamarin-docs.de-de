...
ms.locfileid: "84567956"
---
# <a name="garbage-collection"></a>Automatische Speicherbereinigung
...
Alle Objekte, die auf beiden virtuellen Computern über eine Darstellung verfügen, verfügen Überlebens Dauer, die im Vergleich zu Objekten erweitert werden, die nur innerhalb einer einzelnen VM (z [`System.Collections.Generic.List<int>`](xref:System.Collections.Generic.List%601) . b.) vorhanden sind. GC wird aufgerufen [. Collect](xref:System.GC.Collect) sammelt diese Objekte nicht notwendigerweise, da die xamarin. Android-GC sicherstellen muss, dass auf das Objekt nicht von einem der beiden virtuellen Computer verwiesen wird, bevor Sie es sammeln. 
...
- **Old** : die ursprüngliche Implementierung (als die stabilste der drei). Dies ist die Bridge, die eine Anwendung verwenden sollte, wenn die Pausen `GC_BRIDGE` akzeptabel sind. 
...
Geben Sie an `GC_BRIDGE` , welche Option eine Anwendung verwenden soll `bridge-implementation=old` , `bridge-implementation=new` oder übergeben `bridge-implementation=tarjan` Sie die `MONO_GC_PARAMS` Umgebungsvariable. Dies wird durch Hinzufügen einer neuen Datei zum Projekt mit einer **Buildaktion** von erreicht `AndroidEnvironment` . Beispiel: 
...
[Verwaltete Callable Wrapper](~/android/internals/architecture.md) fügen keine zusätzlichen Instanzmember hinzu. Wenn Sie also über eine [Android. Graphics. Bitmap](xref:Android.Graphics.Bitmap) -Instanz verfügen, die auf ein 10 MB-BLOB des Arbeitsspeichers verweist, weiß die GC von xamarin. Android nicht, dass &ndash; die GC ein 20-Byte-Objekt hat und nicht feststellen kann, dass Sie mit von Android-Lauf Zeit Objekten zugewiesenen Objekten verknüpft ist, die 10 MB Arbeitsspeicher aufrechterhalten. 
...
> [!NOTE]
> Beim Verwerfen von Unterklassen Instanzen müssen Sie *sehr* vorsichtig sein `Java.Lang.Object` .

Beachten Sie beim Aufrufen von die folgenden Richtlinien, um die Möglichkeit einer Speicher Beschädigung zu minimieren `Dispose()` .

#### <a name="sharing-between-multiple-threads"></a>Freigabe zwischen mehreren Threads

Wenn die *Java* -Instanz oder die verwaltete Instanz von mehreren Threads gemeinsam genutzt werden kann, sollte Sie **niemals** *d ( `Dispose()` d) sein*. Zum Beispiel[`Typeface.Create()`](xref:Android.Graphics.Typeface.Create*) 
gibt möglicherweise eine *zwischengespeicherte-Instanz*zurück. Wenn mehrere Threads dieselben Argumente bereitstellen, erhalten Sie *dieselbe* Instanz. Folglich `Dispose()` kann das Löschen der- `Typeface` Instanz von einem Thread andere Threads ungültig machen, was zu `ArgumentException` s von (u. a.) führen kann, `JNIEnv.CallVoidMethod()` da die Instanz von einem anderen Thread verworfen wurde. 

#### <a name="disposing-bound-java-types"></a>Verwerfen von gebundenen Java-Typen

Wenn die Instanz einen gebundenen Java-Typ hat, kann die Instanz verworfen werden, *solange die Instanz* nicht aus verwaltetem Code wieder verwendet wird *und* die Java-Instanz nicht von Threads gemeinsam genutzt werden kann (siehe vorherige `Typeface.Create()` Erörterung). (Diese Bestimmung ist möglicherweise schwierig.) Wenn die Java-Instanz das nächste Mal in verwalteten Code gelangt, wird ein *neuer* Wrapper für Sie erstellt. 
...
Der obige Code ist sicher, weil der von [drawable. kreatefrompath ()](xref:Android.Graphics.Drawables.Drawable.CreateFromPath*) zurückgegebene Peer auf einen frameworkpeer verweist, *nicht* auf einen benutzerpeer. Der `Dispose()` Aufruf am Ende des- `using` Blocks unterbricht die Beziehung zwischen den verwalteten [drawable](xref:Android.Graphics.Drawables.Drawable) -und frameworkdrawable-Instanzen, sodass die Java-Instanz erfasst werden kann, sobald die Android-Laufzeit benötigt. [Drawable](https://developer.android.com/reference/android/graphics/drawable/Drawable.html) Dies wäre *nicht* sicher, wenn Peer Instanz an einen benutzerpeer verwiesen wird. Hier verwenden wir "externe" Informationen, um zu *wissen* , dass `Drawable` nicht auf einen benutzerpeer verweisen kann, und daher ist der- `Dispose()` Rückruf sicher. 

#### <a name="disposing-other-types"></a>Verwerfen anderer Typen 

Wenn die Instanz auf einen Typ verweist, der keine Bindung eines Java-Typs ist (z. b. ein benutzerdefiniertes `Activity` ), **sollten Sie nicht** aufzurufen, wenn `Dispose()` Sie nicht *wissen* , dass der Java-Code überschriebene Methoden für diese Instanz aufruft. [ `NotSupportedException` Wenn dies nicht der Fall](~/android/internals/architecture.md#Premature_Dispose_Calls)ist, führt dies zu einer. 
...
Diese Situation tritt häufig auf, wenn der erste Löschvorgang eines-Objekts bewirkt, dass ein Member NULL wird, und ein nachfolgende Zugriffs Versuch für dieses NULL-Element bewirkt, dass eine Ausnahme ausgelöst wird. Insbesondere das-Objekt `Handle` (das eine verwaltete Instanz mit der zugrunde liegenden Java-Instanz verknüpft) wird beim ersten verwerfen ungültig, aber verwalteter Code versucht weiterhin, auf diese zugrunde liegende Java-Instanz zuzugreifen, auch wenn Sie nicht mehr verfügbar ist (Weitere Informationen zur Zuordnung zwischen Java-Instanzen und verwalteten Instanzen finden Sie unter [verwaltete Aufruf Bare Wrapper](~/android/internals/architecture.md#Managed_Callable_Wrappers) ). 

Eine gute Möglichkeit, diese Ausnahme zu verhindern, besteht darin, in der-Methode explizit zu überprüfen `Dispose` , ob die Zuordnung zwischen der verwalteten Instanz und der zugrunde liegenden Java-Instanz weiterhin gültig ist, d. h., ob das Objekt den Wert `Handle` NULL () hat, bevor Sie auf `IntPtr.Zero` seine Member zugreifen. Beispielsweise greift die folgende `Dispose` Methode auf ein- `childViews` Objekt zu: 
...
Wenn ein anfänglicher Lösch Durchlauf bewirkt `childViews` , dass einen ungültigen `Handle` hat, löst der `for` Schleifen Zugriff einen aus `ArgumentException` . Durch das Hinzufügen einer expliziten `Handle` null-Überprüfung vor dem ersten `childViews` Zugriff verhindert die folgende Methode, dass `Dispose` die Ausnahme auftritt: 
...
Wenn `BadActivity` erstellt wird, enthält das Objekt Diagramm 10004-Instanzen (1 x `BadActivity` , 1X `strings` , 1X, `string[]` gehalten von `strings` , 10000 x Zeichen folgen Instanzen), die *alle* gescannt werden müssen, wenn die- `BadActivity` Instanz gescannt wird. 

Dies kann nachteilige Auswirkungen auf die Sammlungs Zeiten haben, was zu längeren GC-Pausenzeiten führt. 

Sie können den GC unterstützen, indem Sie die Größe von Objekt Diagrammen *verringern* , die von Benutzer Peer Instanzen stammen. Im obigen Beispiel kann dies durch `BadActivity.strings` einen Wechsel zu einer separaten Klasse erfolgen, die nicht von Java. lang. Object erbt: 
