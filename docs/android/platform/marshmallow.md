...
- Mit **API 23/Android 6,0-Bindungen** &ndash; Android 6,0 werden viele neue APIs für die unten beschriebenen neuen Features hinzugefügt. Diese APIs sind für xamarin. Android-Apps verfügbar, wenn Sie auf API-Ebene 23 abzielen. Weitere Informationen zu Android 6,0-APIs finden Sie unter [Android 6,0-APIs](https://developer.android.com/preview/api-overview.html). 

[![Hero-Images von Tablets und Smartphones, die Marshmallow ausführen](marshmallow-images/android-m-hero-sml.png)](marshmallow-images/android-m-hero.png#lightbox)
...
- Die **Verbesserungen** bei der Authentifizierung &ndash; ab Android Marshmallow. Apps können nun Fingerabdrucksensoren verwenden, um Benutzer zu authentifizieren, und mit einer neuen Funktion zum *bestätigen* von Anmelde Informationen wird die Eingabe von Kenn Wörtern minimiert. 

- **App-Verknüpfungen** &ndash; diese Funktion hilft, das Popup Fenster der **App** zu vermeiden, indem Sie Apps automatisch Webdomänen zuordnen. 

- **Direkt Freigabe** &ndash; Sie können *direkte Freigabe Ziele* definieren, die die Freigabe für Benutzer schnell und intuitiv gestalten. Diese Funktion ermöglicht es utoren, Inhalte für andere apps freizugeben. 

- **Sprach Interaktionen** &ndash; diese neue API ermöglicht Ihnen das Erstellen von Funktionen für die Sprachfunktion in Ihrer APP. 

- **4K-Anzeigemodus** &ndash; in Android Marshmallow kann Ihre APP eine 4K-Anzeige Auflösung auf Hardware anfordern, die Sie unterstützt. 

- **Neue Audiofeatures** &ndash; ab Marshmallow unterstützt jetzt das Protokoll "MIDI". Außerdem bietet es neue Klassen zum Erstellen digitaler audioerfassungs-und Wiedergabe Objekte und bietet neue API-Hooks zum Zuordnen von Audiodaten und Eingabegeräten. 

- **Neue Video Features** &ndash; Marshmallow bietet eine neue Klasse, die apps hilft, Audiodaten und Videostreams synchron zu erzeugen. Diese Klasse bietet auch Unterstützung für die dynamische Wiedergabe Rate. 
...
- **Material Design Support Library** &ndash; die neue *Entwurfs Unterstützungs Bibliothek* stellt Entwurfs Komponenten und Muster bereit, mit denen Sie das Aussehen und Verhalten von Material Entwürfen in Ihrer APP vereinfachen können. 
...
[![Konfigurieren eines AVD-Geräts mithilfe eines Nexus 5-Geräts, eines Android 6,0-Ziels und von Intel Atom (x86)](marshmallow-images/android-m-avd.png)](marshmallow-images/android-m-avd.png#lightbox)
...
- **Bestätigen** Sie die Anmelde Informationen &ndash; die Benutzer authentifiziert, je nachdem, wie lange das Gerät entsperrt wurde.
...
Auf Geräten, die die Fingerabdruck Überprüfung unterstützen, können Sie die neue `FingerPrintManager`-Klasse verwenden, um einen Benutzer zu authentifizieren.
...
Zu diesem Zweck verwenden Sie die neue `SetUserAuthenticationValidityDurationSeconds`-Methode des `KeyGenerator`. Verwenden Sie die `CreateConfirmDeviceCredentialIntent`-Methode der `KeyGuardManager`, um den Benutzer in Ihrer APP erneut zu authentifizieren. Weitere Informationen zu diesem neuen Feature in Android Marshmallow finden Sie unter [Confirm Credential](https://developer.android.com/preview/api-overview.html#confirm-credential).
...
App-Entwickler haben eine Auswahl, wenn ein Benutzer auf eine URL tippt: die APP kann entweder einen Browser starten oder einen in-App-Browser verwenden, der auf einem `WebView`basiert. Beide Optionen stellen Herausforderungen dar, &ndash; das Starten des Browsers einen hohen Kontextwechsel ist, der nicht anpassbar ist, während `WebView`s den Zustand nicht mit dem Browser gemeinsam verwenden. Außerdem kann durch die Verwendung von `WebView`s zusätzlicher Wartungsaufwand entstehen. 
...
Android Lollipop hat den [Material Entwurf](https://www.google.com/design/spec/material-design/introduction.html) als neue Entwurfs Sprache eingeführt, um die Android-Darstellung zu aktualisieren (Weitere Informationen zur Verwendung von Material Design in xamarin. Android-Apps finden Sie im [Material](~/android/user-interface/material-theme.md) Design). Mit Android Marshmallow hat Google die *Bibliothek für Android-Entwurfs Unterstützung* eingeführt, mit der App-Entwickler das Aussehen und Verhalten von Material Entwürfen leichter übernehmen können. Diese Bibliothek enthält die folgenden Komponenten:

- **Coordinatorlayout** &ndash; das neue `CoordinatorLayout`-Widget ähnelt, aber leistungsfähiger als ein `FrameLayout`. Sie können `CoordinatorLayout` als Container für untergeordnete Sichten oder als Layout der obersten Ebene verwenden, und es wird ein `layout_anchor` Attribut bereitgestellt, das zum Verankern von Ansichten relativ zu anderen Ansichten verwendet werden kann.

- Das Reduzieren von **Symbolleisten** &ndash; die neue `CollapsingToolbarLayout` ist eine reduzierende App-Leiste, die ein Wrapper für `Toolbar`ist. (Beachten Sie, dass die *App-Leiste* zuvor als *Aktionsleiste*bezeichnet wurde.)

- Unverankerte **Aktions Schaltfläche** &ndash; eine roundschaltfläche, die die primäre Aktion auf der Benutzeroberfläche Ihrer APP angibt.

- **Für Gleit Komma Bezeichnungen zum Bearbeiten von Text** &ndash; wird ein neues `TextInputLayout`-Widget (das `EditText`umschließt) verwendet, um eine Gleit Komma Bezeichnung anzuzeigen, wenn ein Benutzer Text eingibt.

- **Navigationsansicht** &ndash; das neue `NavigationView`-Widget hilft Ihnen, die Navigationsleiste auf eine Weise zu verwenden, die für Benutzer einfacher zu navigieren ist.
...
- **Material Registerkarten** &ndash; das neue `TabLayout`-Widget bietet ein horizontales Layout zum Anzeigen von Registerkarten als Möglichkeit zum Implementieren der Navigation auf oberster Ebene in Ihrer APP.
...
Xamarin stellt eine Beispiel-App zur Verfügung, die die neue Android-Entwurfs Bibliothek in xamarin. Android &ndash; [cheesesquare](https://docs.microsoft.com/samples/xamarin/monodroid-samples/android50-cheesesquare)erläutert.
...
Weitere Informationen zur Entwurfs Bibliothek finden Sie [unter Android Design Support Library](https://android-developers.googleblog.com/2015/05/android-design-support-library.html) im Blog des Android-Entwicklers.
...
- [Android Wearable Library](https://www.nuget.org/packages/Xamarin.Android.Wear) &ndash; diese nuget enthält Google Play Services Bindungen. Die neueste Version der tragbaren Library bietet neue Features (einschließlich einfacherer Navigation für benutzerdefinierte Apps) für die Android Wear-Plattform. 
