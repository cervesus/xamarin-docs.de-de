...
- **`Adapter`** Vergrößert &ndash; Element Layouts (instanziiert den Inhalt einer Layoutdatei) und bindet Daten an Sichten, die in einer angezeigt werden `RecyclerView` . Der Adapter meldet auch Element-Click-Ereignisse.

- **`LayoutManager`**&ndash;Misst und positioniert Element Sichten innerhalb einer `RecyclerView` und verwaltet die Richtlinie für die Ansicht-Wiederverwendung.
...
Die Beziehung zwischen der `RecyclerView` `LayoutManager` -Klasse, der-Klasse und der- `Adapter` Klasse wird im folgenden Diagramm dargestellt:
...
Wie die Abbildung zeigt, `LayoutManager` kann der als Vermittler zwischen dem und dem betrachtet werden `Adapter` `RecyclerView` . Der `LayoutManager` ruft `Adapter` Methoden im Namen von auf `RecyclerView` . Beispielsweise `LayoutManager` Ruft eine-Methode auf, `Adapter` Wenn es an der Zeit ist, eine neue Ansicht für eine bestimmte Element Position in der zu erstellen `RecyclerView` . Vergrößert das `Adapter` Layout für dieses Element und erstellt eine- `ViewHolder` Instanz (nicht angezeigt), um Verweise auf die Ansichten an dieser Position zwischenzuspeichern. Wenn der `LayoutManager` aufruft, `Adapter` um ein bestimmtes Element an das DataSet zu binden, `Adapter` sucht die Daten für dieses Element, ruft Sie aus dem DataSet ab und kopiert sie in die zugeordnete Element Ansicht.

Wenn Sie `RecyclerView` in Ihrer APP verwenden, ist das Erstellen abgeleiteter Typen der folgenden Klassen erforderlich:

- **`RecyclerView.Adapter`**&ndash;Stellt eine Bindung aus dem DataSet Ihrer APP (die für Ihre APP spezifisch ist) für Element Ansichten bereit, die in der angezeigt werden `RecyclerView` . Der Adapter weiß, wie jede Element Ansichts Position in der `RecyclerView` einem bestimmten Speicherort in der Datenquelle zugeordnet wird. Außerdem verarbeitet der Adapter das Layout der Inhalte innerhalb der einzelnen Element Ansichten und erstellt den Ansichts Halter für jede Ansicht. Der Adapter meldet auch Element-Click-Ereignisse, die von der Element Ansicht erkannt werden.
...
5. Die wiederverwendete Ansicht wird der Liste der Elemente in der hinzugefügt `RecyclerView` , die auf dem Bildschirm angezeigt werden.

6. Die wiederverwendete Ansicht wird auf dem Bildschirm angezeigt, wenn der Benutzer einen Bildlauf `RecyclerView` zum nächsten Element in der Liste durchführt. In der Zwischenzeit führt eine andere Ansicht einen Bildlauf aus und wird gemäß den obigen Schritten wieder verwendet.

Zusätzlich zur Wiederverwendung von Element-View `RecyclerView` verwendet auch eine weitere Effizienzoptimierung: Ansichts Halter. Ein *Ansichts Halter* ist eine einfache Klasse, die Ansichts Verweise zwischenspeichert. Jedes Mal, wenn der Adapter eine Element Layoutdatei auffüllt, erstellt er auch einen entsprechenden Ansichts Halter. Der Ansichts Halter verwendet `FindViewById` , um Verweise auf die Sichten in der aufheften Element Layout-Datei zu erhalten. Diese Verweise werden zum Laden neuer Daten in die Ansichten jedes Mal verwendet, wenn das Layout wieder verwendet wird, um neue Daten anzuzeigen.

## <a name="the-layout-manager"></a>Der Layout-Manager

Der LayoutManager ist für das Positionieren von Elementen in der `RecyclerView` Anzeige zuständig. er bestimmt den Präsentationstyp (eine Liste oder ein Raster), die Ausrichtung (ob Elemente vertikal oder horizontal angezeigt werden) und welche Richtungs Elemente angezeigt werden sollen (in normaler Reihenfolge oder in umgekehrter Reihenfolge). Der LayoutManager ist auch für die Berechnung der Größe und Position der einzelnen Elemente in der Wiederverwendung von " **recycleview** " verantwortlich.
...
Um den LayoutManager anzugeben, instanziieren Sie den ausgewählten Layoutmanager, und übergeben Sie ihn an die- `SetLayoutManager` Methode. Beachten Sie, dass Sie angeben *müssen* , dass der LayoutManager &ndash; `RecyclerView` standardmäßig keinen vordefinierten LayoutManager auswählt.
...
Ein ausführliches Beispiel für eine- `ViewHolder` Implementierung wird in [einem grundlegenden recyclerview-Beispiel](~/android/user-interface/layouts/recycler-view/recyclerview-example.md)dargestellt.
...
Der größte Teil des- `RecyclerView` Integrations Codes erfolgt im Adapter. `RecyclerView`erfordert, dass Sie einen von abgeleiteten Adapter angeben `RecyclerView.Adapter` , um auf die Datenquelle zuzugreifen und jedes Element mit Inhalt aus der Datenquelle aufzufüllen.
Da die Datenquelle App-spezifisch ist, müssen Sie Adapter Funktionen implementieren, die den Zugriff auf Ihre Daten verstehen. Der Adapter extrahiert Informationen aus der Datenquelle und lädt Sie in jedes Element in der Auflistung `RecyclerView` .

In der folgenden Abbildung wird veranschaulicht, wie der Adapter Inhalt in einer Datenquelle mithilfe von Ansichts Besitzern den einzelnen Ansichten innerhalb der einzelnen Zeilen Elemente in der zuordnet `RecyclerView` :
...
Der Adapter lädt jede `RecyclerView` Zeile mit Daten für ein bestimmtes Zeilen Element. Bei Zeilen Position *p*sucht der Adapter z. b. die zugeordneten Daten an Position *p* innerhalb der Datenquelle und kopiert diese Daten in das Zeilen Element an Position *p* in der Auflistung `RecyclerView` .
In der obigen Zeichnung verwendet der Adapter z. b. den Ansichts Halter, um die Verweise für `ImageView` und `TextView` an dieser Position zu suchen, sodass er nicht wiederholt `FindViewById` für diese Ansichten aufrufen muss, wenn der Benutzer einen Bildlauf durch die Auflistung durchführt und die Ansichten erneut verwendet.

Wenn Sie einen Adapter implementieren, müssen Sie die folgenden Methoden außer Kraft setzen `RecyclerView.Adapter` :
...
Der Layout-Manager ruft diese Methoden auf, während Elemente innerhalb von positioniert werden `RecyclerView` .
...
`RecyclerView.Adapter`stellt eine Reihe von Methoden bereit, die Sie so ausführen können, dass `RecyclerView` auf Datenänderungen auf möglichst effiziente Weise reagiert:
...
Wenn Sie genau wissen, wie sich das Dataset geändert hat, können Sie die entsprechenden Methoden oben zum Aktualisieren auf `RecyclerView` die effizienteste Weise aufzurufen. Wenn Sie nicht genau wissen, wie sich das Dataset geändert hat, können Sie aufrufen `NotifyDataSetChanged` , was weitaus weniger effizient ist, da `RecyclerView` alle Sichten aktualisieren muss, die für den Benutzer sichtbar sind. Weitere Informationen zu diesen Methoden finden Sie unter [recyclerview. Adapter](https://developer.android.com/reference/android/support/v7/widget/RecyclerView.Adapter.html).
