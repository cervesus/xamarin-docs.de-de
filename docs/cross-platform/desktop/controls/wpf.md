...
- Xamarin. Forms unterstützt die [XAML 2009-Spezifikation](/dotnet/framework/xaml-services/xaml-2009-language-features/). Dies vereinfacht die Definition von Daten, wie z. b. `string`s, `int`s usw., das Definieren von generischen Typen und das Übergeben von Argumenten an Konstruktoren.
...
- Die folgenden Funktionen in Bindungen werden nicht unterstützt:
  - BindingGroupName
  - Bindsdirectlydesource
  - IsAsync
  - MultiBinding
  - Notifyonsourceupdatiert
  - Notifyontargetupveraltet
...
Datenvorlagen werden in xamarin. Forms zum Anpassen des Renderings einer `ListView` Zeile (Zelle) unterstützt. Im Gegensatz zu WPF, das `DataTemplate`s für inhaltsorientierte Steuerelemente verwenden kann, verwendet xamarin. Forms diese derzeit nur für `ListView`. Die Vorlagen Definition kann Inline (der `ItemTemplate`-Eigenschaft zugewiesen) oder als Ressource in einer `ResourceDictionary` definiert werden.
...
3. `DataTemplateSelector` wird ebenfalls unterstützt, wird aber von `DataTemplate` abgeleitet und wird daher direkt der `ItemTemplate`-Eigenschaft (vs.  `ItemTemplateSelector` in WPF).
...
In WPF werden `UserControl`s verwendet, um einen Abschnitt mit zugeordneter Benutzeroberfläche bereitzustellen. In xamarin. Forms verwenden wir die `ContentView` für den gleichen Zweck. Beide unterstützen die Bindung und Einbindung in XAML.
