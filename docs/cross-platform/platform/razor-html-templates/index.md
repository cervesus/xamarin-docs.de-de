...
Die Vorlagenausgabe wird in iOS durch den folgenden C ## Code gerendert:
...
Der Razor-vorlagenmoduls erweitert die standard-HTML-Syntax mit C ##, damit Sie das Layout Express und CSS-Stylesheets und Javascript problemlos integrieren können. Die Vorlage kann eine Modellklasse verweisen, erfolgen kann jeder beliebige benutzerdefinierte Typ und dessen Eigenschaften zugegriffen werden können, direkt aus der Vorlage. Einer der Hauptvorteile ist die Möglichkeit, HTML und C ## Syntax einfach kombinieren.
...
Mehrere C ## Anweisungen geschrieben werden können, vom umgebenden mit `@{}`.
...
Beachten Sie, die Sie problemlos hinzufügen, können eine **cshtml** Razor-Vorlage, um *alle* vorhandenen Xamarin-Projekt, es ist nicht erforderlich, diese Lösungsvorlagen verwenden. iOS-Projekte erfordern ein Storyboard mit Razor verwenden; Fügen Sie einfach ein UIWebView-Steuerelement an einer beliebigen Ansicht programmgesteuert und können Sie Razor-Vorlagen in C ## Code gesamte rendern.
...
Kommunikation mit einer gerenderten Webansicht Rückrufe an C ## erfolgt durch Festlegen der URL für die Webansicht, und klicken Sie dann die Anforderung in C ## an die systemeigene Anforderung zu verarbeiten, ohne erneutes Laden der Webansicht abfangen.
...
Dies versucht, Webansicht an eine URL mit einem benutzerdefinierten Schema zu navigieren, in dem wir Sie vorgenommen haben (`hybrid:`)
...
Wir überprüfen Sie zunächst die URL, die Webansicht versucht wird, zu laden, und wenn sie unsere benutzerdefinierte Schema nicht gestartet (`hybrid:`), wir ermöglichen die Navigation wie gewohnt.

Für unsere benutzerdefinierte URL-Schema, wir behandeln alles, was in der URL zwischen dem Schema und die "?" als der Methodenname (in diesem Fall "UpdateLabel") verarbeitet werden. Alles, was in der Abfragezeichenfolge wird als Parameter für den Methodenaufruf behandelt:
...
UpdateLabel in diesem Beispiel wird nur eine minimale Menge Zeichenfolgenmanipulation im Textbox-Parameter (vorangestellt wird "C ## besagt, dass" "in der Zeichenfolge), und klicken Sie dann einen Rückruf an die Webansicht.

Am Ende unserer URL Behandlung Abbrechen wir die Navigation, damit die Webansicht nicht versucht, Fertig stellen, um unsere benutzerdefinierte URL navigieren.
